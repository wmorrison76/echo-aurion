...
const intentMatchers: Array<{ pattern: RegExp; intent: string; route: string; rationale: string }> = [
  {
  ...
  },
  {
    ...
    rationale: "Mentions of recipes or chefs map to the culinary planning module.",
  },
  {
...
export async function routeIntent(context: IntentContext): Promise<RoutedIntent> {
  ...
  if (context.meshRoot) {
    ...
    if (mesh.recipeToInventory.length) {
      return {
        ...
        rationale: "Defaulted to strongest recipe→inventory edge from cognitive mesh.",
      };
    }
  }
