...
import { get, fmtUsd, fmtPct, sourceTag } from "./api";
...
interface DashboardState {
  ...
  version: AnyObj | null;
  galleyDash: AnyObj | null; // for the doctrine_posture banner
...
export default function LiveDashboard() {
  ...
  // Forecast sparkline data (21d revenue) — must be declared before any early return
  const sparkData = useMemo(() => {
    const fc = forecast21?.forecast || [];
    return fc.map((d: any) => ({
      date: d.date,
      label: d.day_of_week?.slice(0, 3),
      rev: (d.revenue?.total || 0) / 1000, // dollars in $k
    }));
  }, [forecast21]);

  // Capture totals + headroom from ratios
  ...
  return (
    <div className="app-shell">
      ...
      <div className="app-banner">
        <div className="brand">
          <span className="brand-mark">LUCCCA</span>
      ...
      <div className="grid">
        {/* PACE */}
        <Link to={`/dashboard/pace/${propertyId}`} className="tile tile-span-4 rise rise-1" data-testid="tile-pace">
        ...
        {/* CASH RUNWAY */}
        <Link to={`/dashboard/cash-runway/${propertyId}`} className="tile tile-span-4 rise rise-2" data-testid="tile-cash-runway">
          <div className="tile-head">
          ...
          {cashOk ? (
            <>
              ...
              <div className="tile-spacer" />
              <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
                <span className="delta neu">
                  7d {fmtUsd(burn7d, { compact: true })}/d
                </span>
                <span className="delta neu">
                  30d {fmtUsd(burn30d, { compact: true })}/d
                </span>
        ...
        {/* EXCEPTIONS */}
        <Link to={`/dashboard/exceptions/${propertyId}`} className="tile tile-span-4 rise rise-3" data-testid="tile-exceptions">
          <div className="tile-head">
          ...
          <div className="tile-subline">
            {excSummary.red} red · {excSummary.amber} amber · day{" "}
            {exceptions?.date || "—"}
          </div>
          <div className="tile-spacer" />
        ...
        {/* 21-DAY FORECAST */}
        <Link to={`/dashboard/forecast-21/${propertyId}`} className="tile tile-span-8 rise rise-4" data-testid="tile-forecast-21">
          <div className="tile-head">
        ...
        {/* LIFECYCLE / PERIOD CLOSE — links to the May P&L Close deep-dive */}
        <Link to={`/dashboard/period-close/${propertyId}`} className="tile tile-span-4 rise rise-5" data-testid="tile-lifecycle">
          <div className="tile-head">
        ...
        {/* OUTLET CAPTURE GRID — 8 outlets, clickable to deep-dive */}
        <div className="tile tile-span-12 rise rise-6" style={{ cursor: "default" }} data-testid="tile-outlet-capture">
          ...
          <div className="outlet-grid" style={{ gridTemplateColumns: "repeat(4, 1fr)" }}>
            {outlets.map((o: any) => {
              ...
              // Short IDs (≤3 chars) like "ird" are abbreviations → uppercase. Longer names → title case.
              const name =
              ...
              return (
                <Link
                  key={o.outlet_id}
                  to={`/dashboard/outlet/${propertyId}/${o.outlet_id}`}
