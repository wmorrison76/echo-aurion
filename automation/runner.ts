...
function normalizeTaskInputs(inputs: AutomationTaskInput[]): AutomationTaskInput[] {
  return inputs
    .map((input) => ({
...
export async function parseTasksFromText(raw: string): Promise<AutomationTaskInput[]> {
  ...
  if (trimmed.startsWith("[") && trimmed.endsWith("]")) {
    ...
    const results: AutomationTaskInput[] = [];
    for (const entry of parsed) {
      ...
      if (entry && typeof entry === "object") {
        ...
        if (!title) continue;
        const descriptionRaw = (entry as any).description;
  ...
  const tasks: AutomationTaskInput[] = [];
  let current: AutomationTaskInput | null = null;

  const taskHeader = /^(task\s*\d+|step\s*\d+|card\s*\d+)/i;
  const delimiter = /\s*[—\-:\u2013]\s*/; // hyphen, em dash, colon

  for (const line of lines) {
    if (!line) continue;
    if (taskHeader.test(line)) {
      ...
      const stripped = line.replace(taskHeader, "").trim();
      const parts = stripped.split(delimiter);
      const rawTitle = parts.shift()?.trim() ?? stripped;
      const title = rawTitle.trim();
      const descriptionValue = parts.join(" - ").trim();
      current = title
        ? {
            title,
  ...
  if (tasks.length === 0) {
    // fallback: treat each non-empty line as a task title
    return lines
      .filter(Boolean)
