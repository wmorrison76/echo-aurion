...
export const handleWriteFile: RequestHandler = async (req, res) => {
  try {
    const { relPath, contents }:{ relPath:string; contents:string } = req.body || {};
    if (!relPath || typeof contents !== "string") {
      return res.status(400).json({ ok:false, error: "relPath and contents required" });
    }
    const full = path.join(ROOT, relPath);
    if (!isAllowed(full)) return res.status(403).json({ ok:false, error:"Path not allowed" });
    if (isEngineLocked(full)) return res.status(423).json({ ok:false, error:"Engine assets are locked" });
