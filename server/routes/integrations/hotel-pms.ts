/**
 * API: Hotel PMS integration management.
 */
...
router.get("/", async (req: Request, res: Response) => {
  try {
    ...
    const { data, error } = await supabase
      .from("hotel_pms_integrations")
      .select("id, org_id, outlet_id, pms_type, is_active, last_sync_at, created_at")
      .eq("org_id", orgId);
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        pms_type: body.pmsType,
        api_endpoint: body.apiEndpoint ?? null,
        config: body.config ?? {},
        is_active: true,
      })
      .select("id, org_id, outlet_id, pms_type, is_active, created_at")
      .single();
    if (error) throw error;
    return res.json({ success: true, data });
  } catch (error: any) {
