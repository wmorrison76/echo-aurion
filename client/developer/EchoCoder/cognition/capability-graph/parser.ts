...
async function loadManifestFile(filePath: string): Promise<CapabilityManifest | null> {
  ...
  try {
    ...
    throw new Error(`Failed to parse capability manifest ${filePath}: ${(error as Error).message}`);
  }

  if (!data?.id || !data?.name) {
    throw new Error(`Manifest ${filePath} is missing required fields 'id' or 'name'`);
  }
...
export async function loadCapabilityManifests(root: string, options: LoadOptions = {}): Promise<CapabilityManifest[]> {
  const results: string[] = [];
  const ignore = new Set((options.excludeDirs ?? []).map((dir) => path.resolve(root, dir)));
