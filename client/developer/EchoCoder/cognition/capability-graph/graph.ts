...
export function buildCapabilityGraph(manifests: CapabilityManifest[]): CapabilityGraph {
  ...
  for (const manifest of manifests) {
    ...
    for (const input of manifest.inputs) {
      const provider = manifests.find((candidate) => candidate.outputs.includes(input));
      if (provider) {
...
export function summariseGraph(graph: CapabilityGraph) {
  ...
  return {
    ...
    dependencyCount: graph.edges.filter((edge) => edge.kind !== "provides").length,
    owners: Array.from(owners.entries()).map(([owner, count]) => ({ owner, count })),
