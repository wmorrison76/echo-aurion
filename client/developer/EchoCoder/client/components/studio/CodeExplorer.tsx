...
export default function CodeExplorer({
  ...
  const panelFrame =
    "rounded-2xl border border-primary/25 bg-background/60 shadow-[0_28px_65px_rgba(15,118,255,0.18)] backdrop-blur-md";
  const baseFiles = useMemo(() => {
    // Only load root config files to avoid circular dependencies
    ...
    return Object.fromEntries(entries.map(([k, v]) => [k, v as any]));
  }, []);
  ...
  const [sidebarW, setSidebarW] = useState<number>(() =>
    readStoredNumber("code.sidebarW", 220),
  );
  ...
  const [showMinimap, setShowMinimap] = useState<boolean>(() =>
    readStoredFlag("code.minimap", true),
  );
  ...
  const editorTrigger = (action: string, args?: any) => {
    try {
      const editor = editorRef.current?.getEditor();
      if (!editor) {
        console.warn(`Editor not ready for action: ${action}`);
        return;
      }
      editor.trigger("editor", action, args || {});
    } catch (err) {
  ...
  const commands = useMemo(
    () => [
      ...
      },
      {
        id: "find",
        name: "Find in file",
        run: () => editorTrigger("actions.find"),
      },
      {
        id: "replace",
        name: "Replace in file",
        run: () => editorTrigger("editor.action.startFindReplaceAction"),
      },
      {
        id: "rename",
        name: "Rename symbol",
        run: () => editorTrigger("editor.action.rename"),
      },
      {
        id: "searchProject",
  ...
  useEffect(() => {
    ...
    const h = setTimeout(async () => {
      ...
      if (searchAbortRef.current) {
        searchAbortRef.current.abort("New search request");
      }
    ...
    return () => {
      ...
      if (searchAbortRef.current) {
        searchAbortRef.current.abort("Code explorer cleanup");
  ...
  useEffect(() => {
    ...
    const subPos = ed.onDidChangeCursorPosition((e) =>
      setCursor((c) => ({
        ...c,
  ...
  function buildOutline(
    ...
    if (["ts", "tsx", "js", "jsx"].includes(lang)) {
      lines.forEach((ln, i) => {
        let m;
        if ((m = ln.match(/\bfunction\s+([A-Za-z0-9_]+)/)))
          push(`fn ${m[1]}`, i + 1);
        if ((m = ln.match(/\bclass\s+([A-Za-z0-9_]+)/)))
          push(`class ${m[1]}`, i + 1);
        if ((m = ln.match(/\binterface\s+([A-Za-z0-9_]+)/)))
          push(`interface ${m[1]}`, i + 1);
        if ((m = ln.match(/\btype\s+([A-Za-z0-9_]+)/)))
          push(`type ${m[1]}`, i + 1);
        if ((m = ln.match(/\bconst\s+([A-Za-z0-9_]+)\s*=\s*\(/)))
          push(`const ${m[1]}()`, i + 1);
        if ((m = ln.match(/\bconst\s+([A-Za-z0-9_]+)\s*=\s*async\s*\(/)))
          push(`const ${m[1]}()`, i + 1);
        if ((m = ln.match(/\bexport\s+default\b/)))
          push("export default", i + 1);
      });
      ...
      lines.forEach((ln, i) => {
        let m;
        if ((m = ln.match(/^\s*<h([1-6])[^>]*>(.*?)<\/h\1>/i)))
          push(
            `${"#".repeat(parseInt(m[1], 10))} ${m[2].replace(/<[^>]+>/g, "").trim()}`,
            i + 1,
          );
  ...
  async function requestUnlock(): Promise<boolean> {
    ...
    try {
      await fetch("/api/logs/append", {
        ...
        body: JSON.stringify({
          line: `[UNLOCK] until=${new Date(until).toISOString()}`,
        }),
      });
    } catch {}
  ...
  async function sendTerminal() {
    ...
    setTermHistory((h) => [
      ...h,
      { role: "user", text: prompt, time: Date.now() },
    ]);
    setTermBusy(true);
    setTermInput("");
    try {
      ...
      setTermHistory((h) => [
        ...h,
        { role: "assistant", text, time: Date.now() },
      ]);
    } catch (e: any) {
      setTermHistory((h) => [
        ...h,
        { role: "assistant", text: e?.message || String(e), time: Date.now() },
      ]);
    } finally {
      setTermBusy(false);
    }
  ...
  return (
    <>
      <div className="flex gap-4 h-[calc(100vh-96px)] min-h-[620px]">
        ...
        />
        <section className={`${panelFrame} flex-1 min-w-0 flex flex-col`}>
          <div
            ref={tabsRef}
            ...
            {tabs.map((p) => {
              ...
              return (
                <div
                  ...
                  title={
                    locked ? "Protected (click lock to unlock)" : "Editable"
                  }
                >
            ...
            {active ? (
              <>
                <div className="sticky top-0 z-20 flex flex-wrap items-center gap-2 border-b border-primary/30 bg-background/85 px-3 py-2 text-[11px] text-muted-foreground backdrop-blur">
                  {!editorReady && (
                    <span className="text-amber-500">⏳ Editor loading...</span>
                  )}
                  <div className="flex items-center gap-2">
                    {!editing ? (
                      ...
                      <>
                        ...
                        <button
                          className="rounded border px-2 py-0.5 text-xs disabled:opacity-50 disabled:cursor-not-allowed"
                          onClick={async () => {
                            await editorRef.current?.format();
                          }}
                        ...
                        <button
                          ...
                          className="rounded border px-2 py-0.5 text-xs disabled:opacity-50 disabled:cursor-not-allowed"
                          onClick={() => editorTrigger("actions.find")}
                          disabled={!editorReady}
                        >
                          Find
                        </button>
                        ...
                        <button
                          ...
                          className="rounded border px-2 py-0.5 text-xs disabled:opacity-50 disabled:cursor-not-allowed"
                          onClick={() => editorTrigger("editor.action.rename")}
                          disabled={!editorReady}
                        >
                          Rename
                        </button>
                        <button
                          ...
                          className="rounded border px-2 py-0.5 text-xs disabled:opacity-50 disabled:cursor-not-allowed"
                          onClick={() =>
                            editorTrigger("editor.action.revealDefinition")
                          }
                          disabled={!editorReady}
                        >
                          Go to Def
                        ...
                        <button
                          title="Peek Definition (Alt+F12)"
                          className="rounded border px-2 py-0.5 text-xs disabled:opacity-50 disabled:cursor-not-allowed"
                          onClick={() =>
                            editorTrigger("editor.action.peekDefinition")
                          }
                          disabled={!editorReady}
                        >
                          Peek
                        ...
                        <button
                          ...
                          className="rounded border px-2 py-0.5 text-xs disabled:opacity-50 disabled:cursor-not-allowed"
                          onClick={() =>
                            editorTrigger("editor.action.refactor")
                          }
                          disabled={!editorReady}
                        >
                          Extract
                        ...
                        <button
                          ...
                          className="rounded border px-2 py-0.5 text-xs disabled:opacity-50 disabled:cursor-not-allowed"
                          onClick={() =>
                            editorTrigger("editor.action.quickCommand")
                          }
                          disabled={!editorReady}
                        >
                          Palette
                        ...
                        <button
                          ...
                          onClick={async () => {
                            try {
                              ...
                              const role = readStoredString(
                                "studio.role",
                                "editor",
                              );
                              const adminToken = readStoredString(
                                "studio.adminToken",
                                "",
                              );
                              const changes = [
                  ...
                  <div className="ml-auto inline-flex items-center gap-3 text-muted-foreground">
                    <span
                      title="Errors"
                      className={
                        errorCount ? "text-red-600 dark:text-red-400" : ""
                      }
                    >
                      E:{errorCount}
                    </span>
                    <span
                      title="Warnings"
                      className={
                        warnCount ? "text-amber-600 dark:text-amber-400" : ""
                      }
                    >
                      W:{warnCount}
                    </span>
                    <span
                      title="Info"
                      className={
                        infoCount ? "text-sky-600 dark:text-sky-400" : ""
                      }
                    >
                      I:{infoCount}
                    </span>
                  ...
                  <div>
                    <div className="min-h-[420px]">
                      <MonacoCodeEditor
                        ...
                        onChange={(v) => {
                          setDraft(v);
                          if (showOutline) setOutline(buildOutline(v, active));
                        }}
                        minimap={showMinimap}
                        diff={showDiff}
                        allFiles={files}
                          ...
                          {outline.map((s, i) => (
                            <li key={i}>
                              <button
                                ...
                                onClick={() =>
                                  editorRef.current?.reveal(s.line, 1)
                                }
                              >
                                {s.name}{" "}
                                <span className="opacity-60">(L{s.line})</span>
                              </button>
                            </li>
                          ))}
                          {outline.length === 0 && (
                      ...
                      <div className="border-t max-h-56 overflow-auto text-[11px]">
                        <div className="px-3 py-1 flex items-center gap-2">
                          <input
                            ...
                            className="rounded border bg-background px-2 py-1 text-[11px] w-72"
                          />
                          ...
                          {searchResults.map((r, i) => (
                            <li key={i} className="">
                              ...
                              >
                                ...
                                <span className="opacity-60">L{r.line}</span>
                    ...
                    {problemsOpen && (
                      <div className="border-t max-h-40 overflow-auto text-[11px]">
                        ...
                        <ul className="px-3 pb-2 space-y-1">
                          ...
                          {markers.length === 0 && (
                            <li className="text-muted-foreground">No issues</li>
                          )}
                        </ul>
                      </div>
                    )}
                    ...
                    <div className="sticky bottom-0 w-full border-t bg-background/70 backdrop-blur px-2 py-1 text-[11px] flex items-center justify-between">
                      <div>
                        ...
                        {cursor.sel ? ` • Sel ${cursor.sel}` : ""}
                      </div>
                      <div className="opacity-70 truncate max-w-[60%]">
                        {active?.replace(/^\//, "")}
                      </div>
      ...
      {cmdOpen && (
        ...
        >
          <div
            className="mx-auto mt-24 max-w-lg rounded-md border bg-background p-2 text-sm shadow-xl"
