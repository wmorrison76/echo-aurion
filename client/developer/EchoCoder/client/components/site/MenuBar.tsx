...
const menubarConfig = [
  {
    ...
    isActive: ({ pathname }: { pathname: string }) =>
      ["/", "/studio", "/sandbox", "/zaro", "/echo-controls"].includes(
        pathname,
      ),
  },
  {
...
export default function MenuBar() {
  ...
  function handleItemSelect(entry: MenuEntry) {
    try {
      ...
      console.error("MenuBar item selection error:", err);
    }
  }
  ...
  return (
    <div
      data-menubar
      className="sticky top-10 z-50 border-b bg-gradient-to-r from-background/95 via-background/75 to-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/65"
    >
      <div className="w-full flex flex-col gap-0 py-1 px-4">
        <div className="flex flex-col gap-1 lg:flex-row lg:items-center lg:gap-1 lg:overflow-x-auto">
          <Menubar className="min-w-max border-primary/30 bg-background/80 backdrop-blur-sm shadow-sm">
            {menubarConfig.map((menu) => {
              ...
              return (
                <MenubarMenu key={menu.label}>
                  <MenubarTrigger
                    className={cn(
                      "gap-1 rounded-sm px-2 py-1 text-[10px] font-semibold uppercase tracking-[0.2em] whitespace-nowrap",
                      isActive
                        ? "bg-primary/15 text-foreground shadow-inner"
                        : "text-muted-foreground hover:text-foreground",
                    )}
                  >
                    <menu.icon className="h-3 w-3" aria-hidden />
                    {menu.label}
                  </MenubarTrigger>
                  <MenubarContent className="min-w-[280px] max-h-[70vh] overflow-y-auto z-50">
                    {menu.sections.map((section, sectionIndex) => (
                      <div
                        key={`${menu.label}-${sectionIndex}`}
                        className="py-0.5"
                      >
                        {section.label ? (
