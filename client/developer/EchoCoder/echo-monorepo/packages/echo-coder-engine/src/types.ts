export type ScoreWeights = { fit:number; complexity:number; deps:number; perf:number; a11y:number; observability:number; security:number; bundle:number };
export type Score = { total:number; breakdown: Partial<Record<keyof ScoreWeights, number>> };

export type FileChange = { path:string; contents:string; mode?:"create"|"patch" };

export type PlanSpec = { task:string; route?:string; entitlements?:string[]; constraints?:Record<string,unknown> };
export type CandidatePlan = { id:string; title:string; steps:string[]; files:FileChange[]; score:Score };
