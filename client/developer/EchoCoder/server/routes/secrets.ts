...
function roleAllowsMutation(role: string | undefined | null) {
  ...
  return normalized === "admin" || normalized === "lead" || normalized === "lead dev" || normalized === "lead_dev" || normalized === "lead-dev";
}
...
export const handleSecretsSet: RequestHandler = async (req, res) => {
  ...
  const role = (req.body?.role as string | undefined) ?? (req.headers["x-echo-role"] as string | undefined);
  ...
  if (!roleAllowsMutation(role)) {
    return res.status(403).json({ ok: false, error: "Role not authorized to mutate secrets" });
  }
