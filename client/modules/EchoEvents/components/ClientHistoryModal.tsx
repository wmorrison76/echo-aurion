    ...
    issues: [
      {
        ...
        resolution:
          "Additional vegetarian meals prepared, client compensated with 10% discount",
        resolutionDate: new Date("2024-01-15"),
        preventionNotes:
          "Increase vegetarian portions by 20% for corporate events",
        wasResolved: true,
    ...
    issues: [
      {
        ...
        resolution:
          "Audio technician resolved immediately, backup system activated",
      ...
      },
      {
        ...
        severity: "minor",
        description:
          "Delayed setup due to traffic congestion affecting vendor deliveries",
        resolution:
          "Event started 15 minutes late, client was informed and accommodated",
        resolutionDate: new Date("2023-06-10"),
        preventionNotes:
          "Schedule vendor deliveries earlier for high-profile events",
        wasResolved: true,
...
export default function ClientHistoryModal({
  ...
  const [selectedEvent, setSelectedEvent] = useState<ClientEventHistory | null>(
    null,
  );
  const getSatisfactionColor = (level: string) => {
    switch (level) {
      ...
      case "satisfied":
        return "bg-blue-100 text-blue-700 border-blue-200 dark:bg-blue-900/20 dark:text-primary dark:border-blue-800";
      case "neutral":
      ...
      default:
        return "bg-surface text-foreground border-gray-200 dark:bg-gray-900/20 dark:text-gray-300 dark:border-gray-800";
    }
  };
  const getSeverityColor = (severity: string) => {
    switch (severity) {
      ...
      default:
        return "bg-surface text-foreground border-gray-200";
    }
  };
  const averageFeedbackScore =
    eventHistory.reduce((sum, event) => sum + event.guestFeedbackScore, 0) /
    eventHistory.length;
  const totalRevenue = eventHistory.reduce(
    (sum, event) => sum + event.totalRevenue,
    0,
  );
  const totalIssues = eventHistory.reduce(
    (sum, event) => sum + event.issues.length,
    0,
  );
  const resolvedIssues = eventHistory.reduce(
    (sum, event) =>
      sum + event.issues.filter((issue) => issue.wasResolved).length,
    0,
  );
  return (
      ...
      <DialogContent className="glass-panel max-w-6xl max-h-[90vh] overflow-y-auto">
        ...
        <DialogHeader>
          ...
          <DialogTitle className="text-2xl flex items-center">
            {" "}
            <FaCommentDots className="h-6 w-6 mr-3 text-primary" /> Client
            History: {clientName}{" "}
          </DialogTitle>{" "}
          <DialogDescription>
            {" "}
            Complete event history with guest feedback scores and issue
            tracking{" "}
          </DialogDescription>{" "}
        </DialogHeader>{" "}
        {/* Overview Stats */}{" "}
              ...
              <div className="flex items-center justify-between">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold text-green-600">
                    {averageFeedbackScore.toFixed(1)}/10
                  </p>{" "}
                </div>{" "}
        ...
        <Tabs defaultValue="events" className="w-full">
          ...
          <TabsList className="grid w-full grid-cols-3">
            ...
            <TabsTrigger value="events" className="flex items-center">
              {" "}
              <FaCalendarAlt className="h-4 w-4 mr-2" /> Event History{" "}
            </TabsTrigger>{" "}
            <TabsTrigger value="feedback" className="flex items-center">
              {" "}
              <FaStar className="h-4 w-4 mr-2" /> Feedback Trends{" "}
            </TabsTrigger>{" "}
            <TabsTrigger value="issues" className="flex items-center">
              {" "}
              <FaExclamationTriangle className="h-4 w-4 mr-2" /> Issues &
              Resolutions{" "}
            </TabsTrigger>{" "}
          </TabsList>{" "}
            ...
            <Table>
              ...
              <TableHeader>
                ...
                <TableRow>
                  ...
                  <TableHead>Type</TableHead> <TableHead>Attendees</TableHead>{" "}
                  ...
                  <TableRow key={event.id} className="hover:bg-muted/50">
                        ...
                        <div className="flex">
                          ...
                          {Array.from({ length: 5 }, (_, i) => (
                            <FaStar
                              ...
                              className={cn(
                                "h-3 w-3",
                                i < Math.round(event.guestFeedbackScore / 2)
                                  ? "text-yellow-400"
                                  : "text-gray-300",
                              )}
                            />
                    ...
                    <TableCell>
                      ...
                      <Badge
                        className={cn(
                          "text-xs",
                          getSatisfactionColor(event.satisfactionLevel),
                        )}
                      >
                        {" "}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    {eventHistory.map((event, index) => (
                      ...
                      >
                        ...
                        <div className="flex items-center space-x-3">
                          {" "}
                          <Progress
                            value={event.guestFeedbackScore * 10}
                            className="w-24 h-2"
                          />{" "}
                          ...
                          {index > 0 && (
                            <div className="flex items-center">
                              {" "}
                              {event.guestFeedbackScore >
                              eventHistory[index - 1].guestFeedbackScore ? (
              ...
              <Card className="glass-panel apple-button">
                ...
                <CardHeader>
                  ...
                  <CardDescription>
                    Probability of future bookings
                  </CardDescription>{" "}
                </CardHeader>{" "}
                <CardContent>
                  {" "}
                    ...
                    {eventHistory.map((event) => (
                      ...
                      >
                        ...
                        <div className="flex items-center space-x-3">
                          {" "}
                          <Progress
                            value={event.repeatBookingLikelihood}
                            className="w-24 h-2"
                          />{" "}
                          <span className="text-sm font-medium w-12">
                            {event.repeatBookingLikelihood}%
                          </span>{" "}
                        </div>{" "}
                      </div>
                    ))}{" "}
