...
export async function generateOrderGuide(options?: {
  coverCount?: number;
  baselineCovers?: number;
}): Promise<OrderGuideRow[]> {
  const adapters = getPurchRecAdapters();

  // Wrap Promise.all in try-catch to handle any network errors
  let menuItems: MenuItem[] = [];
  let ingredientsMap: Record<string, Ingredient> = {};
  let onHandMap: Record<string, { qtyBase: number; unit: string }> = {};
  let vendorCatalog: VendorCatalogEntry[] = [];

  try {
    const results = await Promise.all([
      adapters.listActiveMenuItems().catch((err) => {
        console.error('[OrderGuide] listActiveMenuItems error:', err);
        return [];
      }),
      loadIngredientsMap().catch((err) => {
        console.error('[OrderGuide] loadIngredientsMap error:', err);
        return {};
      }),
      (adapters.getInventoryOnHand
        ...
        console.error('[OrderGuide] getInventoryOnHand error:', err);
        return {};
      }),
      loadVendorCatalog().catch((err) => {
        console.error('[OrderGuide] loadVendorCatalog error:', err);
        return [];
      }),
    ]);

    [menuItems, ingredientsMap, onHandMap, vendorCatalog] = results;
  } catch (err) {
    console.error('[OrderGuide] Promise.all error:', err);
    // Return empty guide if something catastrophically fails
    return [];
  }
  ...
  const baselineCoversRaw = options?.baselineCovers;
  const baselineCovers =
    typeof baselineCoversRaw === "number" &&
    Number.isFinite(baselineCoversRaw) &&
    baselineCoversRaw > 0
      ? baselineCoversRaw
      : 150;
  const coverCount = options?.coverCount;
  const coverFactor =
    typeof coverCount === "number" &&
    Number.isFinite(coverCount) &&
    coverCount > 0
      ? coverCount / baselineCovers
      : 1;
  const aggregates = await buildIngredientDemand(
    menuItems,
    ingredientsMap,
    coverFactor,
  );
  ...
  for (const menuItem of menuItems.filter((item) => item.active)) {
    ...
    recipe.components.forEach((component: RecipeComponent) => {
      ...
      const demandBase =
        ...
        ) * (Number.isFinite(coverFactor) ? coverFactor : 1);
