...
export interface OutletCaptureDeepDiveProps {
  propertyId: string;
  outletId: string;
  onBack: () => void;
}
...
export default function OutletCaptureDeepDive({
  ...
  const accByH = dash.accuracy || accuracy?.accuracy || {};
  ...
  return (
    <div className="app-shell">
      ...
      <div className="drilldown">
        ...
        {dash.doctrine_posture && <div className="callout">{dash.doctrine_posture}</div>}
                  ...
                  <tbody>
                    {Object.entries(accByH).map(([h, v]: [string, any]) => (
                      <tr key={h}>
                        ...
                        <td>{v?.smape != null ? `${(v.smape * 100).toFixed(1)}%` : "—"}</td>
                        <td style={{ color: "var(--dim)" }}>{v?.band || "n/a"}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={histChart} margin={{ top: 8, right: 12, left: -10, bottom: 0 }}>
                ...
                <Tooltip
                  ...
                  formatter={(v: any) => [`${v}%`, ""]}
                />
                <Legend iconType="square" wrapperStyle={{ fontSize: 11, color: "#8a8478", paddingTop: 4 }} />
        ...
        >
          ...
          <span>generated · {dash.generated_at} · property · {propertyId}</span>
        </div>
      </div>
    </div>
  );
