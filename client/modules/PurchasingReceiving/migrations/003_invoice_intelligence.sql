-- Adaptive OCR & enrichment infrastructure
...
alter table invoices
  ...
  add column if not exists glare_score numeric(5,2),
  add column if not exists noise_score numeric(5,2),
  add column if not exists template_id uuid references invoice_vendor_templates(id),
  add column if not exists normalized_payload jsonb,
  add column if not exists variance_score numeric(5,2),
  add column if not exists requires_review boolean default false,
  add column if not exists review_status text default 'pending' check (review_status in ('pending','in_progress','resolved','dismissed')),
...
alter table invoice_lines
  add column if not exists normalized_qty numeric,
  add column if not exists normalized_uom text,
  add column if not exists canonical_item_code text,
