import {
  ...
  summariseGraph,
} from "../capability-graph/graph";
import {
  CapabilityManifest,
  loadCapabilityManifests,
} from "../capability-graph/parser";
...
async function ensureManifests(
  ...
  if (!options.root) {
    throw new Error(
      "Either manifests or root must be provided to build the cognitive mesh",
    );
  }
  return loadCapabilityManifests(options.root, {
    excludeDirs: ["node_modules", "dist"],
  });
}
export async function buildCognitiveMesh(
  options: MeshOptions,
): Promise<MeshSummary> {
  ...
  for (const node of graph.nodes.values()) {
    heatmap[node.owner ?? "unassigned"] =
      (heatmap[node.owner ?? "unassigned"] ?? 0) + 1;
