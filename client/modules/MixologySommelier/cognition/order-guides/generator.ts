...
export function buildOrderGuidePlan(
  ...
  const eventName =
    options.eventName ?? inferEventName(transcript) ?? "Service";
  ...
  for (const recipeKey of recipes) {
    ...
    for (const ingredient of recipe.ingredients) {
      ...
      const casesToOrder =
        toOrder > 0 ? Math.ceil(toOrder / ingredient.caseSize) : 0;
...
export function renderOrderGuide(plan: OrderGuidePlan): string {
  const storeroomLines = plan.storeroom.length
    ? plan.storeroom
        .map(
          (line) =>
            `• ${line.name} (${line.sku}) – pull ${line.pullFromStoreroom.toFixed(1)}${line.unit} (${line.caseLabel})`,
        )
        .join("\n")
    ...
    ? plan.vendorOrders
        .map((line) => {
          ...
          const casesText =
            cases > 0
              ? `${cases} case(s)`
              : `${line.orderFromVendor.toFixed(1)}${line.unit}`;
...
function inferCovers(transcript: string): number | null {
  const match = transcript.match(
    /(\d{2,4})\s*(guests|people|covers|servings?)/i,
  );
