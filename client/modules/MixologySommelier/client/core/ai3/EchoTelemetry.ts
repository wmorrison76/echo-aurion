...
export class EchoTelemetry {
  ...
  getAverageSentiment() {
    const vals = this.packets
      .filter((p) => typeof p.sentiment === "number")
      .map((p) => p.sentiment!);
    if (!vals.length) return 0;
    return vals.reduce((acc, value) => acc + value, 0) / vals.length;
  }
  getPerformanceStats() {
    const fps = this.packets
      .filter((p) => typeof p.fps === "number")
      .map((p) => p.fps!);
    const latency = this.packets
      .filter((p) => typeof p.latencyMs === "number")
      .map((p) => p.latencyMs!);
    return {
