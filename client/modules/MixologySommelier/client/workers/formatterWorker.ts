/// <reference lib="webworker" /> // Prettier runs in worker to avoid blocking the UI
...
function parserFrom(
  ...
  if (language === "typescript" || /\.(ts|tsx)$/.test(lower))
    return "typescript";
  ...
  if (language === "markdown" || /\.(md|markdown)$/.test(lower))
    return "markdown";
  return "babel";
}
