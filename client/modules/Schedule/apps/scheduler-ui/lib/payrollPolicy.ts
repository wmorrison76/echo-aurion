...
export function applyPolicy({
  ...
  for (const h of daily) {
    ...
    if (rem > dtThresh) {
      const dth = rem - dtThresh;
      rem -= dth;
      dt += dth;
      components.push({
        kind: "doubletime",
        hours: dth,
        rate: baseRate * policy.dt_multiplier,
        amount: dth * baseRate * policy.dt_multiplier,
      });
    }
    if (rem > dailyOT) {
      const oth = rem - dailyOT;
      rem -= oth;
      ot += oth;
      components.push({
        kind: "overtime",
        hours: oth,
        rate: baseRate * policy.ot_multiplier,
        amount: oth * baseRate * policy.ot_multiplier,
      });
    }
    if (rem > 0) {
      reg += rem;
