...
export default function AttendanceTracker({
  ...
  const [selectedEmpId, setSelectedEmpId] = React.useState<string | null>(
    employees[0]?.id || null,
  );
  ...
  const StatusButton = ({
    ...
    <button
      ...
      className={`px-3 py-2 rounded text-sm font-medium transition-colors ${active ? `bg-${color}-500 text-white` : `border border-input hover:bg-muted`}`}
    >
      {" "}
      {label}{" "}
    </button>
      ...
      <div className="rounded-2xl border border-border/60 bg-background/70 p-4 backdrop-blur">
        <div className="mb-3 space-y-1">
          ...
          <p className="text-sm text-muted-foreground">
            Track attendance, punctuality, and exceptions inline with the active schedule.
          </p>
        </div>
        {body}
      </div>
