...
function compute(employees: EmployeeRow[]) {
  ...
  const tips = employees.reduce(
    (s, e) =>
      s + Object.values(e.shifts).reduce((t, c) => t + Number(c.tip ?? 0), 0),
    0,
  );
...
export default function LaborSummary({ employees }: Props) {
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-1 gap-3">
      ...
      {card("SPLH", totalHours > 0 ? `$${splh.toFixed(2)}` : "$0.00")}{" "}
      {card(
        "Budget / Variance",
        `$${budget.toFixed(2)} • ${variance >= 0 ? "+" : ""}$${variance.toFixed(2)}`,
        variance >= 0
          ? ((<span className="text-emerald-500">Under/On budget</span>) as any)
          : ((<span className="text-red-500">Over budget</span>) as any),
      )}{" "}
    </div>
  );
}
