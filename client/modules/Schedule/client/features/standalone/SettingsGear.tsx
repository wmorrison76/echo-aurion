...
export default function SettingsGear({
  ...
  const s = React.useMemo(() => loadSettings(), [open]);
  const [time24, setTime24] = React.useState(s.timeFormat24h);
  const [startDay, setStartDay] = React.useState<number>(s.startDay);
  const profiles = listProfiles(employees);
  const [selId, setSelId] = React.useState<string>(profiles[0]?.id || "");
  const [p, setP] = React.useState<EmployeeProfile | null>(
    selId
      ? getProfile(selId, {
          id: selId,
  ...
  const [adminEntered, setAdminEntered] = React.useState(false);
  const [adminTry, setAdminTry] = React.useState("");
  const applyAdmin = () => {
      ...
      <DialogContent className="max-w-2xl">
        ...
        <div className="grid md:grid-cols-2 gap-4 text-sm">
          ...
          <section className="space-y-2">
            ...
            <div className="text-xs text-muted-foreground">
              {" "}
              Click “This week” after saving to jump to the new week
              layout.{" "}
            </div>{" "}
          </section>{" "}
          <section className="space-y-2">
