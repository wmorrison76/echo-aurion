...
export default function TimeOffPanel({
  ...
  const [requests, setRequests] = React.useState<TimeOffRequest[]>(() => {
    try {
      return JSON.parse(
        localStorage.getItem("shiftflow:timeoff-requests") || "[]",
      );
    } catch {
      return [];
    }
  });
  const [ptoBalance, setPtoBalance] = React.useState<Record<string, number>>(() => {
    try {
  ...
  const RequestCard = ({ req }: { req: TimeOffRequest }) => {
    const statusColor = {
      pending: "bg-yellow-50 border-yellow-200 dark:bg-amber-950/30 dark:border-amber-900/40",
      approved: "bg-green-50 border-green-200 dark:bg-emerald-950/30 dark:border-emerald-900/40",
      rejected: "bg-red-50 border-red-200 dark:bg-red-950/30 dark:border-red-900/40",
    }[req.status];
    const statusIcon = {
        ...
        <div className="flex items-center justify-between mb-3">
          ...
          <div>
            ...
            <span className="inline-block px-2 py-1 bg-background/50 rounded text-xs font-medium">
              {" "}
              {timeOffTypes.find((t) => t.id === req.type)?.label ||
                req.type}{" "}
            </span>{" "}
            {req.reason && (
  ...
  const body = (
    <div className="grid gap-4">
          ...
          <div className="grid grid-cols-4 gap-3">
            {" "}
            <div className="border rounded p-3 bg-yellow-50 border-yellow-200 dark:bg-amber-950/30 dark:border-amber-900/40">
              {" "}
              <div className="text-xs font-medium text-yellow-700 dark:text-amber-300">
                Pending
              </div>{" "}
              <div className="text-2xl font-bold text-yellow-600 dark:text-amber-400 mt-1">
                {stats.pending}
              </div>{" "}
            </div>{" "}
            <div className="border rounded p-3 bg-green-50 border-green-200 dark:bg-emerald-950/30 dark:border-emerald-900/40">
              {" "}
              <div className="text-xs font-medium text-green-700 dark:text-emerald-300">
                Approved
              </div>{" "}
              <div className="text-2xl font-bold text-green-600 dark:text-emerald-400 mt-1">
                {stats.approved}
              </div>{" "}
            </div>{" "}
            <div className="border rounded p-3 bg-red-50 border-red-200 dark:bg-red-950/30 dark:border-red-900/40">
              {" "}
            ...
            <div className="border rounded p-3 bg-blue-50 border-blue-200 dark:bg-sky-950/30 dark:border-sky-900/40">
              {" "}
              <div className="text-xs font-medium text-blue-700 dark:text-sky-300">
                Days Approved
              </div>{" "}
                ...
                {(["all", "pending", "approved", "rejected"] as const).map(
                  (status) => (
                    <button
                      ...
                      className={`px-3 py-1 rounded text-sm font-medium transition-colors ${filterStatus === status ? "bg-primary text-primary-foreground" : "border border-input hover:bg-muted"}`}
                    >
                      {" "}
                      {status === "all" && "All"}{" "}
            ...
            <div className="space-y-4">
              ...
              <div className="border rounded p-4">
                ...
                <h3 className="font-semibold mb-3 flex items-center gap-2">
                  {" "}
                  <Calendar className="w-5 h-5" /> Time-off Calendar{" "}
                </h3>{" "}
    ...
    return (
      <div className="space-y-4 rounded-2xl border border-border/60 bg-background/70 p-4 backdrop-blur">
        <div className="space-y-1">
          <h3 className="text-base font-semibold">Time Off & PTO Management</h3>
          <p className="text-sm text-muted-foreground">
            Review requests, balances, and upcoming coverage impact inline with the schedule.
          </p>
        </div>
        {body}
      </div>
