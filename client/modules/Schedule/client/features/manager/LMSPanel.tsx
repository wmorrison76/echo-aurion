...
export default function LMSPanel({ employees }: { employees?: EmployeeRow[] }) {
  ...
  const [activeTab, setActiveTab] = React.useState<
    "stations" | "standards" | "hours" | "compliance"
  >("standards");
  const [stations, setStations] = React.useState<StationRange[]>(() => {
    try {
  ...
  const [cfg, setCfg] = React.useState(getComplianceConfig());
  useEffect(() => {
    try {
  ...
  const [presets, setPresets] = React.useState<string[]>(() => loadPresets(dept));
  useEffect(() => {
    setPresets(loadPresets(dept));
  }, [dept]);
  const [std, setStd] = React.useState<Record<string, StandardRule[]>>(() =>
  ...
  const [maxTo, setMaxTo] = React.useState<number>(() =>
        ...
        <div className="flex gap-2 border-b mb-4">
          ...
          {tabs.map((tab) => (
            <button
              ...
              className={`px-4 py-2 font-medium text-sm border-b-2 transition-colors ${activeTab === tab.id ? "border-primary text-primary" : "border-transparent text-muted-foreground hover:text-foreground"}`}
            >
              {" "}
              {tab.label}{" "}
            </button>
            ...
            <div>
              ...
              <div className="grid gap-2">
                ...
                {stations.map((st, i) => (
                  <div
                    key={i}
                    className="flex items-center gap-2 bg-muted/30 p-3 rounded"
                  >
                    {" "}
                    <input
        ...
        {activeTab === "standards" && (
          <section className="grid gap-4">
            ...
            <div>
              {" "}
              <h3 className="font-semibold mb-3">
                Labor Standards (3-Panel Editor)
              </h3>{" "}
                ...
                <div className="border rounded p-3">
                  ...
                  <div className="grid gap-1 max-h-48 overflow-auto mb-2">
                    ...
                    {(presets || []).map((p) => (
                      <button
                        key={p}
                        className={`text-left border rounded px-2 py-1 text-sm transition-colors ${selectedLeft === p ? "bg-primary text-primary-foreground" : "hover:bg-muted"}`}
                  ...
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <label className="text-xs">
                      ...
                      <input
                        ...
                        value={step}
                        onChange={(e) =>
                          setStep(Math.max(1, Number(e.target.value) || 50))
                        }
                      />{" "}
                    </label>{" "}
                    <label className="text-xs">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setMaxTo(
                            Math.max(step, Number(e.target.value) || 650),
                          )
                        }
                      />{" "}
                    </label>{" "}
                      ...
                      <thead>
                        ...
                        <tr>
                          ...
                          {ranges().map((r, i) => (
                            <th
                              key={i}
                              className="p-1 text-center whitespace-nowrap text-[10px]"
                            >
                              {" "}
                              {i === 0
                                ? `0-${step}`
              ...
              <div className="bg-muted/30 rounded p-4">
                ...
                <div className="grid grid-cols-3 gap-3 text-sm">
                  ...
                  <div>
                    ...
                    <div
                      className={`text-lg font-semibold ${preview.issues.length === 0 ? "text-green-600" : "text-red-600"}`}
                    >
                      {" "}
                  ...
                  <div>
                    ...
                    <div className="text-lg font-semibold">
                      {preview.overtimeHours.toFixed(1)}h
                    </div>{" "}
                  </div>{" "}
                  <div>
                    {" "}
                    <div className="text-muted-foreground">
                      Predictability Pay Hours
                    </div>{" "}
                    <div className="text-lg font-semibold">
                      {preview.predictabilityPayHours.toFixed(1)}h
                    </div>{" "}
                  </div>{" "}
                </div>{" "}
