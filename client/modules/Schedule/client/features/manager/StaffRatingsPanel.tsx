...
interface Rating {
  ...
  rater: string;
  category: string;
  score: number; // 1-5 comments?: string;
}
interface EmployeePerformance {
  ...
  totalRatings: number;
  ratings: Rating[];
  attendance: number; // % punctuality: number; // % reliability:"excellent" |"good" |"fair" |"poor";
}
export default function StaffRatingsPanel({
  ...
  const [activeTab, setActiveTab] = React.useState<"overview" | "detail" | "history">(
    "overview",
  );
  const [ratingScore, setRatingScore] = React.useState(5);
  const [ratingCategory, setRatingCategory] = React.useState("overall");
  const [ratingComment, setRatingComment] = React.useState("");
  const performanceData: EmployeePerformance[] = React.useMemo(() => {
  ...
  const selectedEmployee = selectedEmpId
    ? performanceData.find((e) => e.empId === selectedEmpId)
    : null;
  const StarRating = ({
    score,
    onChange,
    ...
    <div className="flex gap-1">
      ...
      {[1, 2, 3, 4, 5].map((n) => (
        <button
          ...
          disabled={readOnly}
          className={`transition-colors ${n <= score ? "text-yellow-500" : "text-gray-300 dark:text-zinc-600"}`}
        >
          {" "}
      ...
      <div className="space-y-4">
        <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
          ...
          <div className="border rounded-lg p-4 bg-muted/30">
            ...
            <div className="text-2xl font-bold mt-1">
              {performanceData.length
                ? (
                    performanceData.reduce((sum, emp) => sum + emp.avgRating, 0) /
                    performanceData.length
                  ).toFixed(1)
                : "0.0"}
            </div>
          ...
          <div className="border rounded-lg p-4 bg-muted/30">
            ...
            <div className="text-2xl font-bold mt-1">{selectedEmployee?.name || "—"}</div>
          </div>
        </div>
                    ...
                    <div className="flex items-start justify-between mb-4">
                      ...
                      <div>
                        ...
                        <div className="text-sm text-muted-foreground mt-1">
                          {" "}
                          {selectedEmployee.totalRatings} ratings •{" "}
                          {selectedEmployee.reliability}{" "}
                        </div>{" "}
                      </div>{" "}
                      <div className="text-center">
                        {" "}
                        <div className="text-3xl font-bold">
                          {selectedEmployee.avgRating.toFixed(1)}
                        </div>{" "}
                        <StarRating
                          score={Math.round(selectedEmployee.avgRating)}
                          readOnly
                        />{" "}
                      </div>{" "}
                    </div>{" "}
                    <div className="grid grid-cols-2 gap-3 mb-4">
                      ...
                      <div>
                        ...
                        <div className="flex items-end gap-2 mt-1">
                          {" "}
                          <div className="text-2xl font-bold">
                            {selectedEmployee.attendance.toFixed(0)}%
                          </div>{" "}
                          <div
                            className="h-12 bg-green-200 dark:bg-emerald-900/50 rounded"
                            style={{
                              width: `${selectedEmployee.attendance * 0.6}px`,
                            }}
                          />{" "}
                        </div>{" "}
                      </div>{" "}
                      <div>
                        ...
                        <div className="flex items-end gap-2 mt-1">
                          {" "}
                          <div className="text-2xl font-bold">
                            {selectedEmployee.punctuality.toFixed(0)}%
                          </div>{" "}
                          <div
                            className="h-12 bg-blue-200 dark:bg-sky-900/50 rounded"
                            style={{
                              width: `${selectedEmployee.punctuality * 0.6}px`,
                            }}
                          />{" "}
                        </div>{" "}
                      </div>{" "}
                  ...
                  <div className="border rounded-lg p-4">
                    ...
                    <div className="grid gap-3">
                      ...
                      <Button
                        onClick={() => {
                          alert(
                            `Rating submitted for ${selectedEmployee.name}`,
                          );
                          setRatingComment("");
                          setRatingScore(5);
                        }}
