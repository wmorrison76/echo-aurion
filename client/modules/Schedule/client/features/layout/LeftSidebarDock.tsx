...
export default function LeftSidebarDock() {
  const items = [
    ...
    { icon: <FileText className="w-4 h-4" />, label: "Reports Hub" },
  ...
  return (
    <aside className="hidden lg:block w-56 border-r bg-background/50 p-2 space-y-1">
      ...
      {items.map((it) => (
        <div
          key={it.label}
          className={`flex items-center gap-2 px-2 py-1 rounded-md text-sm ${it.active ? "bg-primary/10 text-primary" : "text-muted-foreground hover:bg-accent hover:text-accent-foreground"}`}
        >
          {" "}
          {it.icon}
