...
export default function TimesheetReview({
  ...
  useEffect(() => {
    ...
    const fetchPayroll = async () => {
      ...
      let url = `${apiUrl}?outlet_id=${encodeURIComponent(outlet)}`;
  ...
  const summary = useMemo(() => {
    ...
    const initialSummary = { reg: 0, ot: 0, dt: 0, pay: 0 };
  ...
  return (
    <div className="rounded-md border bg-background p-3 space-y-3 text-foreground">
      ...
      <div className="flex items-center justify-between border-t pt-2">
        ...
        <div className="flex gap-2">
          {["adp", "quickbooks", "paychex", "paylocity", "gusto", "dayforce", "ukg"].map(
            (v) => (
              <button
                key={v}
