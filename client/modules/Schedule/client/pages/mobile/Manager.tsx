...
export default function ManagerMobile() {
  const [weekISO, setWeekISO] = React.useState(startOfWeekISO());
  const [state, setState] = React.useState<ScheduleState | null>(null);
  const [activeTab, setActiveTab] = React.useState<
    "overview" | "schedule" | "labor" | "alerts"
  >("overview");
  const [editingEmpId, setEditingEmpId] = React.useState<string | null>(null);
  const [editDay, setEditDay] = React.useState<DayKey | null>(null);
  const [newShiftValue, setNewShiftValue] = React.useState("");
  useEffect(() => {
  ...
  const metrics = useMemo(() => {
    ...
    const staffedCount = state.employees.filter(
      (e) => weeklyHours(e) > 0,
    ).length;
  ...
  const alerts = useMemo(() => {
    ...
    const unscheduledCount = state.employees.filter(
      (e) => weeklyHours(e) === 0,
    ).length;
    if (otCount > 0) {
      issues.push({
    ...
    <div className="min-h-screen bg-gradient-to-br from-background via-background to-background md:hidden">
      ...
      <div className="p-3 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <div
            className={`border rounded p-3 ${metrics.cost > metrics.budget ? "bg-red-50" : "bg-green-50"}`}
          >
            {" "}
            <DollarSign
              className={`w-4 h-4 mb-1 ${metrics.cost > metrics.budget ? "text-red-600" : "text-green-600"}`}
            />{" "}
            <div className="text-xs text-muted-foreground">Cost</div>{" "}
            <div
              className={`text-xl font-bold ${metrics.cost > metrics.budget ? "text-red-600" : "text-green-600"}`}
            >
              {" "}
        ...
        {alerts.length > 0 && (
          <div className="space-y-2">
            ...
            {alerts.map((alert, i) => (
              <div
                key={i}
                className={`border rounded p-3 ${alert.severity === "danger" ? "bg-red-50 border-red-200" : alert.severity === "warning" ? "bg-yellow-50 border-yellow-200" : "bg-blue-50 border-blue-200"}`}
              >
                {" "}
                <div className="flex gap-2">
                  {" "}
                  <AlertCircle
                    className={`w-5 h-5 flex-shrink-0 mt-0.5 ${alert.severity === "danger" ? "text-red-600" : alert.severity === "warning" ? "text-yellow-600" : "text-primary"}`}
                  />{" "}
        ...
        <div className="flex gap-2 border-b bg-muted/30 rounded-lg overflow-hidden">
          ...
          {(["overview", "schedule", "labor", "alerts"] as const).map((tab) => (
            <button
              ...
              className={`flex-1 py-2 px-1 text-xs font-medium transition-colors ${activeTab === tab ? "bg-primary text-primary-foreground" : "text-muted-foreground"}`}
            >
              {" "}
            ...
            <div className="border rounded-lg p-4">
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="border-t pt-2 mt-2 flex justify-between">
                  ...
                  <span
                    className={`font-bold ${metrics.cost > metrics.budget ? "text-red-600" : "text-green-600"}`}
                  >
                    {" "}
                    ${metrics.cost.toFixed(0)}{" "}
                  </span>{" "}
        ...
        {activeTab === "schedule" && state && (
          <div className="space-y-3">
            ...
            <div className="border rounded-lg overflow-hidden">
              {" "}
              <div className="bg-muted/50 p-3 font-semibold text-sm">
                Staff Schedule
              </div>{" "}
                  ...
                  <div key={emp.id} className="p-3">
                    ...
                    <div className="grid grid-cols-4 gap-1 text-xs">
                      ...
                      {(DAYS.slice(0, 4) as DayKey[]).map((day) => {
                        ...
                        const hasShift =
                          shift && (shift.in || shift.out || shift.value);
                        return (
                          <div
                            key={day}
                            className={`p-1 rounded text-center ${hasShift ? "bg-primary/10" : "bg-muted"}`}
                          >
                            {" "}
                            {day}{" "}
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <Dialog>
                  ...
                  <DialogContent>
                    ...
                    <Input placeholder="Role" /> <Button>Add</Button>{" "}
                  </DialogContent>{" "}
                </Dialog>{" "}
                <Button size="sm" variant="outline">
                  {" "}
          ...
          <div className="space-y-3">
            ...
            {alerts.length === 0 ? (
              ...
              alerts.map((alert, i) => (
                <div
                  key={i}
                  className={`border rounded-lg p-4 ${alert.severity === "danger" ? "bg-red-50 border-red-200" : alert.severity === "warning" ? "bg-yellow-50 border-yellow-200" : "bg-blue-50 border-blue-200"}`}
                >
                  {" "}
