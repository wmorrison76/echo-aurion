      ...
      shifts: z.record(
        ...
        z.object({
          ...
          range: z
            ...
            .optional()
            .default(null),
          in: z.string().optional().default(""),
          out: z.string().optional().default(""),
          position: z.string().optional().default(""),
          breakMin: z.number().optional().default(0),
          tip: z.number().optional().default(0),
        }),
      ),
    }),
  ),
  managerMessage: z.string().optional().default(""),
  headerLogoDataUrl: z.string().optional().default(""),
});
...
export function hoursForCell(c: ShiftCell | undefined): number {
  if (!c) return 0;
...
export function dayTotals(employees: EmployeeRow[]): Record<DayKey, number> {
  return DAYS.reduce(
    (acc, d) => {
      acc[d] = employees.reduce((sum, e) => {
        const shift = e?.shifts?.[d];
        return sum + hoursForCell(shift);
      }, 0);
      return acc;
    },
...
export function exportCSV(state: ScheduleState): string {
  ...
  const rows = state.employees.map((e) => {
    const vals = DAYS.map((d) => {
      const c = e.shifts?.[d];
      if (!c) return "";
      return c.in || c.out || c.value
        ? `${c.in ?? ""}-${c.out ?? ""}`.trim() || c.value
        : "";
    });
  ...
  const footer = [
    "Totals",
    ...DAYS.map((d) => dayTotals(state.employees)[d].toFixed(2)),
    "",
  ];
...
export function scheduleScopeKey(scope?: ScheduleScope) {
  try {
    const outletId = scope?.outletId?.trim() || localStorage.getItem("shiftflow:outlet") || "Main";
    const deptId = scope?.deptId?.trim() || localStorage.getItem("shiftflow:dept") || "Default";
    return `${STORAGE_KEY_BASE}:${outletId}::${deptId}`;
  } catch {
    return STORAGE_KEY_BASE;
  }
}

function storageKey(scope?: ScheduleScope) {
  return scheduleScopeKey(scope);
}

export function loadSchedule(scope?: ScheduleScope): ScheduleState | null {
  try {
    const s = localStorage.getItem(storageKey(scope));
    if (!s) return null;
    const parsed = JSON.parse(s);
    const res = ScheduleSchema.safeParse(parsed);
    if (!res.success) return null;
    // Ensure all employees have complete shift data for all days
    const data = res.data as ScheduleState;
    return {
      ...data,
      employees: data.employees.map((emp) => ({
        ...
        shifts: DAYS.reduce(
          (acc, day) => {
            const existingShift = emp.shifts[day];
            acc[day] = {
              value: existingShift?.value ?? "",
              range: existingShift?.range ?? null,
              in: existingShift?.in ?? "",
              out: existingShift?.out ?? "",
              position: existingShift?.position ?? "",
              breakMin: existingShift?.breakMin ?? 0,
              tip: existingShift?.tip ?? 0,
            };
            return acc;
          },
          {} as Record<DayKey, ShiftCell>,
...
export function saveSchedule(state: ScheduleState, scope?: ScheduleScope) {
  try {
    localStorage.setItem(storageKey(scope), JSON.stringify(state));
  } catch {}
}

function formatDateRange(weekStartISO: string) {
...
function dataUrlToImageExtension(dataUrl: string): "png" | "jpg" | "jpeg" | "gif" | "webp" {
  const match = dataUrl.match(/^data:image\/(png|jpe?g|gif|webp);base64,/i);
  if (!match) return "png";
  const ext = match[1].toLowerCase();
  if (ext === "jpeg") return "jpg";
  return ext as "png" | "jpg" | "gif" | "webp";
}

export interface ScheduleExcelOptions {
  title: string;
  contextLabel?: string;
  managerMessage?: string;
  logoDataUrl?: string;
...
export async function exportScheduleWorkbook(
  ...
  sheet.columns = [
    ...
    ...DAYS.map((day) => ({ key: day, width: 15 })),
  ];

  const range = formatDateRange(weekStartISO);
  ...
  sheet.getCell(2, 1).value = [options.contextLabel, range].filter(Boolean).join(" · ");
  sheet.getRow(2).font = { italic: true, size: 11 };
  sheet.getRow(2).alignment = { vertical: "middle", wrapText: true };
  sheet.getRow(2).height = 20;

  sheet.mergeCells(3, 1, 3, totalColumns);
  if (options.managerMessage) {
    sheet.getCell(3, 1).value = options.managerMessage;
  ...
  const headerRow = sheet.addRow(["Employee", ...DAYS]);
  headerRow.font = { bold: true };
  headerRow.fill = { type: "pattern", pattern: "solid", fgColor: { argb: "FFE5E7EB" } };

  if (options.logoDataUrl) {
    const imageId = workbook.addImage({
      base64: options.logoDataUrl.replace(/^data:image\/[^;]+;base64,/, ""),
      extension: dataUrlToImageExtension(options.logoDataUrl),
    });
  ...
  employees.forEach((emp) => {
    const row = sheet.addRow([
      ...
      ...DAYS.map((day) => {
        ...
        if (!cell) return "";
        return cell.in || cell.out || cell.value
          ? `${cell.in ?? ""}-${cell.out ?? ""}`.trim() || cell.value
          : "";
      }),
    ]);
    row.eachCell((cell) => {
