...
const POST = (path: string, body: any = {}) =>
  fetch(`${API}/api/chef-carissa${path}`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) }).then(r => r.json());
const GET = (path: string) => fetch(`${API}/api/chef-carissa${path}`).then(r => r.json());
...
const CARISSA_COLOR = "#ec4899";

export default function ChefCarissaTrainingPanel() {
    ...
    try {
      ...
      mediaRecorder.onstop = async () => {
        ...
        try {
          ...
          const res = await fetch(`${API}/api/chef-carissa/transcribe`, { method: "POST", body: formData });
    ...
    <div className="flex flex-col h-full overflow-hidden" style={{ ...FONT, background: BG, color: "#e2e8f0" }}
      ...
      <div className="flex items-center gap-1 px-4 py-1.5 shrink-0" style={{ borderBottom: `1px solid ${BORDER}` }}>
        <div className="w-6 h-6 rounded-full flex items-center justify-center mr-1" style={{ background: "rgba(34,197,94,0.15)" }}>
          <ChefHat className="w-3 h-3" style={{ color: CARISSA_COLOR }} />
        </div>
        <span className="text-[11px] font-semibold text-white mr-3">Chef Carissa</span>
      ...
      {activeView === "review" ? (
          ...
          <div className="p-4" style={{ borderBottom: `1px solid ${BORDER}` }}>
            <div className="flex items-center gap-2 mb-3">
              <div className="w-8 h-8 rounded-full flex items-center justify-center" style={{ background: "rgba(34,197,94,0.15)", border: `1px solid rgba(34,197,94,0.3)` }}>
                <ChefHat className="w-4 h-4" style={{ color: CARISSA_COLOR }} />
              </div>
              <div>
                <div className="text-[12px] font-bold text-white">Chef Carissa</div>
                <div className="text-[9px]" style={{ ...MONO, color: CARISSA_COLOR }}>TRAINING MODULE</div>
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <button onClick={() => startSession(beoSearch ? parseInt(beoSearch) : undefined)}
                ...
                style={{ background: "rgba(34,197,94,0.12)", color: CARISSA_COLOR, border: `1px solid rgba(34,197,94,0.2)` }}
        ...
        <div className="flex items-center gap-3 px-5 py-3" style={{ borderBottom: `1px solid ${BORDER}` }}>
          ...
          <ChefHat className="w-5 h-5" style={{ color: CARISSA_COLOR }} />
          <div>
            <div className="text-[13px] font-semibold text-white">Chef CarissaGenoa Training</div>
          ...
          {messages.length === 0 && !session && (
            <div className="flex flex-col items-center justify-center h-full gap-4">
              <div className="w-16 h-16 rounded-full flex items-center justify-center"
                ...
                <ChefHat className="w-8 h-8" style={{ color: CARISSA_COLOR }} />
              </div>
              <div className="text-center">
                <div className="text-lg font-semibold text-white mb-1">Chef CarissaGenoa Training Module</div>
                <div className="text-[12px] max-w-md mx-auto" style={{ color: "rgba(148,163,184,0.5)" }}>
                  Upload a BEO and Chef Carissa will walk you through every detail — recipes, ordering, production timelines, firing sequences, equipment management. Like having a seasoned Executive Chef training you one-on-one.
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3 mt-4">
                {[
                  ...
                  <button key={m.mode} onClick={() => { setSelectedMode(m.mode); startSession(); }}
                    ...
                    <m.icon className="w-4 h-4 mt-0.5 shrink-0" style={{ color: CARISSA_COLOR }} />
                    <div>
              ...
              <div className="max-w-[75%]">
                {msg.role === "assistant" && (
                  <div className="flex items-center gap-1.5 mb-1">
                    <ChefHat className="w-3 h-3" style={{ color: CARISSA_COLOR }} />
                    <span className="text-[9px] font-medium" style={{ color: CARISSA_COLOR }}>Chef Carissa</span>
                  </div>
                )}
          ...
          {sending && (
            <div className="flex items-center gap-2">
              ...
              <div className="flex gap-1">
                <span className="w-1.5 h-1.5 rounded-full animate-bounce" style={{ background: CARISSA_COLOR, animationDelay: "0ms" }} />
                <span className="w-1.5 h-1.5 rounded-full animate-bounce" style={{ background: CARISSA_COLOR, animationDelay: "150ms" }} />
                <span className="w-1.5 h-1.5 rounded-full animate-bounce" style={{ background: CARISSA_COLOR, animationDelay: "300ms" }} />
              </div>
              <span className="text-[10px]" style={{ color: "rgba(34,197,94,0.5)" }}>Chef Carissa is thinking...</span>
            </div>
          )}
        </div>
          ...
          <div className="px-5 pb-4 pt-2" style={{ borderTop: `1px solid ${BORDER}` }}>
            <div className="flex gap-2">
              ...
              <input type="text" value={input}
                ...
                placeholder={recording ? "Listening..." : transcribing ? "Transcribing audio..." : "Type or speak your response to Chef Carissa..."}
