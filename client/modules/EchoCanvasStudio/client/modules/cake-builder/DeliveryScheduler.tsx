...
export default function DeliveryScheduler({
  ...
  const deliveryObj = schedule.deliveryDate
  ...
  const isRushed =
    daysUntilEvent !== null && daysUntilEvent < DELIVERY_LEAD_DAYS;
    ...
    <Card className="w-full">
      ...
      <CardContent className="space-y-4">
        ...
        {daysUntilEvent !== null && (
          ...
          >
            {isPastDeadline ? (
              <div>🚨 EVENT DATE HAS PASSED - Cannot accept order</div>
            ) : isRushed ? (
              <div>
                ⚠️ RUSH ORDER - {daysUntilEvent} days until event (minimum{" "}
                {DELIVERY_LEAD_DAYS}
                days recommended). $50 rush fee applies.
              </div>
            ) : (
              <div>✓ Normal timeline - {daysUntilEvent} days until event</div>
            )}
          </div>
        )}
