...
function buildChanges(prev: BEODocument, next: BEODocument) {
  ...
  if (
    JSON.stringify(prev.timeline || null) !==
    JSON.stringify(next.timeline || null)
  ) {
    changes.push({
      path: "timeline",
      label: "Timeline",
      previous: prev.timeline,
      current: next.timeline,
    });
  }
  if (
    JSON.stringify(prev.setup || null) !== JSON.stringify(next.setup || null)
  ) {
    changes.push({
      path: "setup",
      label: "Setup",
