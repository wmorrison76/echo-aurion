...
import { useAppData } from "../../../../../Culinary/client/context/AppDataContext";
import { useTranslation } from "../../../context/LanguageContext";
import { usePageToolbar } from "../../../../../Culinary/client/context/PageToolbarContext";
...
const DishAssemblyWorkspace: React.FC = () => {
  ...
  const recipeSummaries = React.useMemo(() => {
  ...
  const [componentRows, setComponentRows] =
    React.useState<DishComponentRow[]>(createInitialRows);
  ...
  const [serverNotes, setServerNotes] = React.useState("");
  const [serviceware, setServiceware] = React.useState("");
  const [allergenRows, setAllergenRows] = React.useState<AllergenRow[]>([]);
  const [pairingRows, setPairingRows] = React.useState<PairingRow[]>([]);
  const [posMappings, setPosMappings] = React.useState<PosMapping[]>(() =>
    defaultPosMappings(),
  );
  const [posConnectOpen, setPosConnectOpen] = React.useState(false);
  const [selectedStationIds, setSelectedStationIds] = React.useState<string[]>([]);
  const [selectedPrinterIds, setSelectedPrinterIds] = React.useState<string[]>([]);
  const [activeComponentId, setActiveComponentId] = React.useState<string | null>(
    null,
  );
  ...
  const [routingTab, setRoutingTab] = React.useState<"stations" | "printers">(
    "stations",
  );
  const [stationQuery, setStationQuery] = React.useState("");
  const [printerQuery, setPrinterQuery] = React.useState("");
  ...
  const popularityDisplay = Number.isFinite(popularityScore)
    ...
    : null;
  const stations = React.useMemo(() => listStations(), [listStations]);
  const printers = React.useMemo(() => listPrinters(), [listPrinters]);
  const selectedStationDetails = React.useMemo(
    () => stations.filter((station) => selectedStationIds.includes(station.id)),
    [stations, selectedStationIds],
  );
  const selectedPrinterDetails = React.useMemo(
    () => printers.filter((printer) => selectedPrinterIds.includes(printer.id)),
    [printers, selectedPrinterIds],
  );
  const recommendedPrinterIds = React.useMemo(() => {
  ...
  const recommendedPrinterSet = React.useMemo(
  ...
  const stationOptions = React.useMemo<RoutingOption[]>(() => {
  ...
  const printerOptions = React.useMemo<RoutingOption[]>(() => {
    return filteredPrinters.map(
      (printer) =>
        ({
          id: printer.id,
          label: printer.name,
  ...
  const stationSummaryItems = React.useMemo<RoutingSummaryItem[]>(() => {
  ...
  const printerSummaryItems = React.useMemo<RoutingSummaryItem[]>(() => {
    return selectedPrinterDetails.map(
      (printer) =>
        ({
          id: printer.id,
          label: printer.name,
  ...
  const handleAssignRecipe = React.useCallback(
    (rowId: string, recipeId: string | null) => {
      setComponentRows((rows) =>
        rows.map((row) => {
          ...
          return { ...row, recipeId, label: baselineLabel, notes: row.notes };
        }),
      );
    },
    [recipeSummaries],
  );
  const addComponentRow = React.useCallback(() => {
    setComponentRows((rows) => [...rows, createBlankRow()]);
  }, []);
  const removeComponentRow = React.useCallback((rowId: string) => {
    ...
    setToolbar({
      ...
      items: [
        ...
        },
        {
          ...
          ariaLabel: t("dishAssembly.actions.posConnect", "POS Connect"),
      ...
      try {
        ...
        if (added > 0) {
          toast({
            title: "Image stored",
            description:
              "Uploaded dish imagery is now available in the gallery.",
          });
        } else {
          toast({
  ...
  const lastStationIndexRef = React.useRef<number | null>(null);
  const lastPrinterIndexRef = React.useRef<number | null>(null);
  const handleStationToggle = React.useCallback(
  ...
  const handleClearStations = React.useCallback(() => {
    setSelectedStationIds([]);
    lastStationIndexRef.current = null;
  }, []);
  const handleClearPrinters = React.useCallback(() => {
    setSelectedPrinterIds([]);
    lastPrinterIndexRef.current = null;
  }, []);
  React.useEffect(() => {
  ...
  const focusRecipe = React.useCallback((rowId: string) => {
  ...
  React.useEffect(() => {
    ...
    setComponentRows((rows) => {
      ...
      const updated = { ...target, recipeId: incomingId, label: resolvedLabel };
  ...
  const navigateToRecipeSearch = React.useCallback(
    (recipeId: string) => {
              ...
              <div className="space-y-3">
                ...
                <div className="relative rounded-xl border border-primary/30 bg-muted/20 p-2">
                  ...
                  <div className="mt-3 flex flex-wrap items-center gap-2">
                    ...
                    >
                      {" "}
                      <RotateCcw className="mr-1.5 h-4 w-4" /> Use recipe
                      media{" "}
                    </Button>{" "}
                    <Button
                      size="sm"
          ...
          <Card className="border-primary/30 bg-background/95 shadow-lg">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                >
                  {" "}
                  <RefreshCcw className="mr-1.5 h-4 w-4" /> Clear workspace{" "}
                </Button>{" "}
              </div>{" "}
            </CardContent>{" "}
          </Card>{" "}
          ...
          <Card className="border-primary/30 bg-background/95 shadow-lg">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex flex-wrap items-center gap-2 text-[11px] uppercase tracking-[0.32em] text-muted-foreground">
                ...
                >
                  {" "}
                  {selectedStationIds.length} station
                  {selectedStationIds.length === 1 ? "" : "s"}{" "}
                </Badge>{" "}
                <Badge
                  variant="outline"
                  className="border-primary/40 text-primary"
                >
                  {" "}
                  {selectedPrinterIds.length} printer
                  {selectedPrinterIds.length === 1 ? "" : "s"}{" "}
                </Badge>{" "}
              </div>{" "}
              <Tabs
                value={routingTab}
            ...
            <CardContent className="space-y-3 text-sm">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-base font-semibold">
                  {" "}
                  {menuPrice ||
                    formatCurrencyValue(totalCost * 3.25, currency)}{" "}
                </span>{" "}
              </div>{" "}
