...
type DocMeta = { doc: OperationsDoc; status: DocStatus; lastUpdated: Date };
...
export default function OperationsDocsWorkspace() {
  ...
  const docMeta = React.useMemo<DocMeta[]>(() => operationsDocs.map(evaluateDoc), []);
  const pinnedSet = React.useMemo(() => new Set(pinnedIds), [pinnedIds]);
  const categories = React.useMemo(() => {
    return Array.from(new Set(operationsDocs.map((doc) => doc.category))).sort(
  ...
  const stats = React.useMemo(() => buildStats(docMeta), [docMeta]);
  const filteredMeta = React.useMemo(() => {
    const term = search.trim().toLowerCase();
    return docMeta.filter((meta) => {
      if (
        selectedCategories.length &&
        !selectedCategories.includes(meta.doc.category)
      ) {
        return false;
      }
  ...
  const sortedMeta = React.useMemo(() => {
  ...
  React.useEffect(() => {
    ...
    if (
      !selectedDocId ||
      !sortedMeta.some((meta) => meta.doc.id === selectedDocId)
    ) {
...
function determineStatus(doc: OperationsDoc, lastUpdated: Date): DocStatus {
  ...
  const alertWindow =
    doc.auditWindowDays ?? Math.max(2, Math.round(doc.cadenceDays * 0.3));
  if (daysUntil < 0) {
    return {
      id: "past-due",
      label: "Past due",
...
function buildStats(meta: DocMeta[]) {
  ...
  return { total, dueSoon, pastDue, highRisk, attachments, avgCompletion };
}
function compareDocs(
  ...
  const pinnedDiff =
    Number(pinnedSet.has(a.doc.id)) - Number(pinnedSet.has(b.doc.id));
  if (pinnedDiff !== 0) {
    return -pinnedDiff;
  }
  switch (sortOption) {
  ...
  return (
    <Card
      className={cn(
        ...
        tone === "warning" &&
          "border-amber-400/80 bg-amber-50/40 dark:border-amber-400/30 dark:bg-amber-900/10",
        tone === "destructive" &&
          "border-red-500/70 bg-red-50/50 dark:border-red-500/30 dark:bg-red-900/10",
      )}
    >
      {" "}
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-3 text-xs text-muted-foreground">
        ...
        <InfoRow
          icon={CalendarClock}
          label="Next review"
          value={formatRelative(status.daysUntilReview, status.nextReview)}
        />{" "}
...
type InfoRowProps = { icon: typeof FileText; label: string; value: string };
...
type StatusBadgeProps = { status: DocStatus };
      ...
      <Card className="sticky top-[88px] h-fit border border-dashed border-border/70 bg-card/50 backdrop-blur-sm">
        ...
        <CardContent>
          ...
          <p className="text-sm text-muted-foreground">
            {" "}
            Adjust your filters or pick a document from the library to review
            workflows, ownership, and compliance history.{" "}
          </p>{" "}
        </CardContent>{" "}
      </Card>
    );
    ...
    <Card className="sticky top-[88px] h-fit border border-border/70 bg-card/80 backdrop-blur-sm">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid gap-3 rounded-lg border border-border/60 bg-background/50 p-3 text-xs">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Tooltip>
              ...
              <TooltipContent>
                {formatRelative(status.daysUntilReview, status.nextReview)}
              </TooltipContent>{" "}
            </Tooltip>{" "}
        ...
        <div className="space-y-3">
          {" "}
          <h4 className="text-xs font-semibold uppercase tracking-[0.32em] text-muted-foreground">
            Metrics
          </h4>{" "}
          <div className="space-y-2">
            ...
            <MetricProgress
              label="Completion"
              value={doc.metrics.completionRate}
            />{" "}
            <MetricProgress label="Adoption" value={doc.metrics.adoption} />{" "}
            <MetricProgress
              label="Coverage"
              value={doc.metrics.coverage}
            />{" "}
          </div>{" "}
        </div>{" "}
        <div className="space-y-2">
...
type MetricProgressProps = { label: string; value: number };
    ...
    <Card className="border border-dashed border-border/70 bg-card/60 p-6 text-center">
      ...
      <CardContent className="space-y-4 p-0">
        ...
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground">
            {" "}
            Try adjusting your filters or clearing search to see the full
            operations playbook.{" "}
          </p>{" "}
        </div>{" "}
        <Button
        ...
        >
          {" "}
          <Filter className="h-4 w-4" aria-hidden /> Reset filters{" "}
        </Button>{" "}
      </CardContent>{" "}
    </Card>
  );
