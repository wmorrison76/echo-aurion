...
export default function ProductionSection(){
  ...
  const [roles, setRoles] = React.useState<Role[]>(()=> readLS(LS_ROLES, [ { id: uid(), name: "Baker" }, { id: uid(), name: "Chocolates & Confections" } ]));
  const [staff, setStaff] = React.useState<Staff[]>(()=> readLS(LS_STAFF, [ { id: uid(), name: "Mike", roleId: undefined } ]));
  const [outlets, setOutlets] = React.useState<Outlet[]>(()=> readLS(LS_OUTLETS, [ { id: uid(), name: "Banquets", type: "Banquets", orderCutoff:"14:00" }, { id: uid(), name: "Cafe", type: "Outlet", orderCutoff:"12:00" } , { id: uid(), name: "Custom Cakes", type: "Custom Cakes", orderCutoff:"10:00" } ]));
  const [orders, setOrders] = React.useState<Order[]>(()=> readLS(LS_ORDERS, []));
  const [ordersTrash, setOrdersTrash] = React.useState<DeletedOrder[]>(()=> readLS(LS_ORDERS_TRASH, []));
  const [logs, setLogs] = React.useState<{ id:string; ts:number; kind:string; message:string; actorId?:string; actorName?:string }[]>(()=> readLS(LS_LOGS, []));
  const [tasks, setTasks] = React.useState<Task[]>(()=> readLS(LS_TASKS, []));
  const [raw, setRaw] = React.useState<RawItem[]>(()=> readLS(LS_INV_RAW, [ { id: uid(), name: "Flour", unit: "kg", onHand: 50, par: 30, location:"Row A • Shelf 1 • Bin 1" }, { id: uid(), name: "Chocolate", unit: "kg", onHand: 20, par: 10, location:"Row B • Shelf 2 • Bin 3" } ]));
  const [fin, setFin] = React.useState<FinishedItem[]>(()=> readLS(LS_INV_FIN, [ { id: uid(), name: "Croissant", unit: "pcs", onHand: 80, par: 120, location:"Freezer 1 • Rack 2 • Tray A" }, { id: uid(), name: "Chocolate Bonbons", unit: "pcs", onHand: 120, par: 150, location:"Freezer 2 • Rack 1 • Tray C" } ]));

  const [date, setDate] = React.useState<string>(()=> new Date().toISOString().slice(0,10));

  const [currentUserId, setCurrentUserId] = React.useState<string | null>(()=> readLS(LS_SESSION_USER, null));
  const currentUser = React.useMemo(()=> staff.find(s=> s.id===currentUserId) || null, [staff, currentUserId]);

  const [taskDialogOpen, setTaskDialogOpen] = React.useState(false);
  const [taskDraft, setTaskDraft] = React.useState<Task | null>(null);
  const [editingId, setEditingId] = React.useState<string | null>(null);
  const [prepOpen, setPrepOpen] = React.useState(false);
  const [prepStart, setPrepStart] = React.useState<string>(()=> date);
  const [prepEnd, setPrepEnd] = React.useState<string>(()=> date);
  const [selectedOutletIds, setSelectedOutletIds] = React.useState<string[]>(()=> []);
  const [enabledGroups, setEnabledGroups] = React.useState<Record<string, boolean>>({});

  const [menu, setMenu] = React.useState<{ open: boolean; x: number; y: number; orderId?: string }>(()=>({ open:false, x:0, y:0 }));
  const [guideOutlet, setGuideOutlet] = React.useState<Outlet | null>(null);
  const [guideStart, setGuideStart] = React.useState<string>(()=> date);
  const [guideUntil, setGuideUntil] = React.useState<string>(()=> date);

  const [orderDialogOpen, setOrderDialogOpen] = React.useState(false);
  const [orderEditingId, setOrderEditingId] = React.useState<string | null>(null);
  const [orderDraft, setOrderDraft] = React.useState<{ id:string; outletId:string; date:string; time:string; notes:string; lines: OrderLine[] } | null>(null);

  const [quickOpen, setQuickOpen] = React.useState(false);
  const [quickDraft, setQuickDraft] = React.useState<{ outletId:string; date:string; time:string; lines: { id:string; item:string; qty:number; unit:string; finishedItemId?:string }[] } | null>(null);
  const [quickRecurring, setQuickRecurring] = React.useState(false);
  const [quickDays, setQuickDays] = React.useState<number[]>([]); // 0=Sun..6=Sat
  const [quickUntil, setQuickUntil] = React.useState<string>(()=> date);

  const [multiAssignOpen, setMultiAssignOpen] = React.useState(false);
  const [multiAssignOrderId, setMultiAssignOrderId] = React.useState<string | null>(null);
  const [multiAssign, setMultiAssign] = React.useState<string[]>([]);


  const [confirmDelOpen, setConfirmDelOpen] = React.useState(false);
  const [pendingDeleteOrderId, setPendingDeleteOrderId] = React.useState<string | null>(null);
  const [deleteReason, setDeleteReason] = React.useState("");
  const [deleteError, setDeleteError] = React.useState("");


  const calRef = React.useRef<HTMLDivElement | null>(null);
  const dragRef = React.useRef<{ id: string; type: 'move'|'resize'; startY: number; startX: number; startMin: number; endMin: number } | null>(null);

  React.useEffect(()=> writeLS(LS_ROLES, roles), [roles]);
  React.useEffect(()=> writeLS(LS_STAFF, staff), [staff]);
  React.useEffect(()=> writeLS(LS_OUTLETS, outlets), [outlets]);
  React.useEffect(()=> writeLS(LS_ORDERS, orders), [orders]);
  React.useEffect(()=> writeLS(LS_ORDERS_TRASH, ordersTrash), [ordersTrash]);
  React.useEffect(()=> writeLS(LS_LOGS, logs), [logs]);
  React.useEffect(()=> writeLS(LS_TASKS, tasks), [tasks]);
  React.useEffect(()=> writeLS(LS_INV_RAW, raw), [raw]);
  React.useEffect(()=> writeLS(LS_INV_FIN, fin), [fin]);
  React.useEffect(()=> writeLS(LS_SESSION_USER, currentUserId), [currentUserId]);
  ...
  const dayTasks = React.useMemo(()=> tasks.filter(t=> t.dateISO===date).sort((a,b)=> a.start.localeCompare(b.start)), [tasks, date]);
  ...
  const outletsById = React.useMemo(()=> Object.fromEntries(outlets.map(o=>[o.id,o])), [outlets]);
  ...
  const combinedEvents = React.useMemo<CalendarEvent[]>(()=>{
  ...
  const prepGroups = React.useMemo(()=>{
  ...
  const prepGroupsModal = React.useMemo(()=>{
  ...
  React.useEffect(()=>{ const el = timelineRef.current; if(!el) return; const ro = new ResizeObserver(()=> setTimelineWidth(el.clientWidth)); ro.observe(el); setTimelineWidth(el.clientWidth); return ()=> ro.disconnect(); },[]);
  const pxPerMinX = React.useMemo(()=> timelineWidth>0 ? timelineWidth/(24*60) : 2, [timelineWidth]);
  ...
  React.useEffect(()=>()=>{ window.removeEventListener('pointermove', onDrag as any); },[]);

  // Orders map and line counts
  const ordersById = React.useMemo(()=> Object.fromEntries(orders.map(o=> [o.id, o])), [orders]);
  const orderLinesCount = React.useMemo(()=> { const m: Record<string, number> = {}; for(const o of orders){ m[o.id]=o.lines.length; } return m; }, [orders]);
  ...
  const [overflowGroupId, setOverflowGroupId] = React.useState<number|null>(null);
