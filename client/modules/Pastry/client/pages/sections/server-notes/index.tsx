...
export default function ServerNotesSection() {
  ...
  const loadSavedNote = (note: ServerNote) => {
    ...
    toast({
      ...
      description: t("serverNotes.toast.loaded.description", undefined, {
        title: noteTitle,
      }),
    });
  };
  ...
  useEffect(() => {
    ...
    setCurrentNote((prev) => {
      const existingIds = new Set(
        prev.selectedRecipes.map((entry) => entry.recipe.id),
      );
      ...
      ids.forEach((id) => {
        ...
        additions.push({ recipe, order: baseOrder + additions.length });
      });
      if (!additions.length) {
        return prev;
      }
      ...
      toast({
        ...
        description:
          addedCount === 1
            ...
            : t("serverNotes.toast.recipesAdded.other", undefined, {
                count: addedCount,
              }),
      });
    }
  ...
  const panelSurfaceClass =
    "overflow-hidden rounded-3xl border border-white/70 bg-background shadow-[0_32px_90px_-48px_rgba(15,23,42,0.35)] backdrop-blur-xl transition-shadow dark:border-[#c8a97e]/15 dark:bg-card dark:shadow-[0_0_70px_rgba(56,189,248,0.35)]";
        ...
        <div className="space-y-6">
          ...
          <header className="flex flex-wrap items-center justify-between gap-2 border-b pb-3">
            ...
            <div className="flex gap-2">
              ...
              >
                ...
                {t("serverNotes.actions.newDocument")}{" "}
              </Button>{" "}
              <Button
                variant="outline"
                size="sm"
              ...
              >
                ...
                {t("export.serverNotes.button", "Export")}{" "}
              </Button>{" "}
              <Button
                variant="outline"
                size="sm"
              ...
              >
                {" "}
                <Download className="h-4 w-4" />{" "}
                {t("export.cooksRecipes.button", "Export Recipes")}{" "}
              </Button>{" "}
              <Button
                variant="outline"
                size="sm"
              ...
              >
                {" "}
                <Download className="h-4 w-4" />{" "}
                {t("export.allergenSheet.button", "Export Allergens")}{" "}
              </Button>{" "}
            </div>{" "}
          </header>{" "}
            ...
            <div className="mt-3 grid gap-3 md:grid-cols-3 xl:gap-4 2xl:gap-6">
              ...
              {WALKTHROUGH_STEPS.map((step, index) => {
                ...
                return (
                  <div
                    key={step.titleKey}
                    className="flex items-start gap-2.5 rounded-2xl border border-white/60 bg-background p-3 shadow-sm backdrop-blur-sm transition-colors dark:border-[#c8a97e]/25 dark:bg-card"
                  >
                    {" "}
                    <span className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full border border-white/80 bg-background text-[11px] font-semibold uppercase tracking-[0.2em] text-muted-foreground shadow-sm dark:border-[#c8a97e]/30 dark:bg-surface">
                      {" "}
                      {index + 1}{" "}
                    </span>{" "}
                ...
                <div className="flex flex-wrap items-center justify-between gap-2.5">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <CardTitle className="text-sm font-semibold">
                      {t("serverNotes.panels.configuration")}
                    </CardTitle>{" "}
                  </div>{" "}
                </div>{" "}
              </CardHeader>{" "}
          ...
          <section>
            ...
            <div className="grid items-stretch gap-6 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
                    ...
                    <CardTitle className="flex items-center justify-between text-base">
                      ...
                      <Badge variant="outline">
                        ...
                        {note.selectedRecipes.length === 1
                          ...
                          : t(
                              "serverNotes.saved.badge.recipes.other",
                              undefined,
                  ...
                  <CardContent className="flex flex-1 flex-col justify-between space-y-4 px-6 pb-6 pt-4 text-sm">
                    ...
                    <div className="space-y-1 text-muted-foreground">
                      ...
                      <div>
                        {" "}
                        <strong>
                          {t("serverNotes.saved.labels.company")}
                        </strong>{" "}
                        {note.companyName || "—"}{" "}
                      </div>{" "}
                      {note.outletName && (
                        <div>
                          {" "}
                          <strong>
                            {t("serverNotes.saved.labels.outlet")}
                          </strong>{" "}
                          {note.outletName}{" "}
                        </div>
                      )}{" "}
                      <div>
                        {" "}
                        <strong>
                          {t("serverNotes.saved.labels.distribution")}
                        </strong>
                        {""}{" "}
                        {new Date(
                      ...
                      <div>
                        {" "}
                        <strong>
                          {t("serverNotes.saved.labels.layout")}
                        </strong>{" "}
                        {note.layout.name}{" "}
                      </div>{" "}
                      <div>
              ...
              {savedNotes.length === 0 && (
                <div className="col-span-full rounded-3xl border border-dashed border-white/70 bg-background py-12 text-center text-sm text-muted-foreground shadow-inner backdrop-blur-sm dark:border-[#c8a97e]/20 dark:bg-card">
                  {" "}
