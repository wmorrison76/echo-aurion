...
type BaseUnitResult = { value: number; unit: string; dimension: Dimension };
type TokenTarget = {
  nameTokens: string[];
...
function parseIngredient(value: string): NormalizedIngredient {
  ...
  const categories = Array.from(
    new Set(baseTokens.flatMap((token) => INGREDIENT_CATEGORIES[token] ?? [])),
  );
...
const YIELD_REFERENCE_INDEX: YieldReferenceEntryInternal[] =
  YIELD_REFERENCE_DATA.map((entry) => {
...
function findReferenceYield(target: TokenTarget): BaseYieldMatch | null {
  ...
  for (const entry of YIELD_REFERENCE_INDEX) {
    ...
    const methodOverlap = countSharedTokens(
      target.prepTokens,
      entry.prepTokens,
    );
    if (methodOverlap) score += methodOverlap * 7;
    const descriptorToBase = countSharedTokens(
      target.descriptorTokens,
      entry.tokens,
    );
...
function evaluateHeuristicYield(target: TokenTarget): BaseYieldMatch {
  ...
  if (hasAny(TRIM_TOKENS)) {
    ...
    return { percent: clampPercent(percent), reason, ruleId };
  }
...
export function combineYields(
  ...
  if (base == null && chef == null) return { percent: null, source: "none" };
