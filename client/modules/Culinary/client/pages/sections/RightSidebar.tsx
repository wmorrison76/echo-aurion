...
export default function RightSidebar(props: RightSidebarProps) {
  ...
  const panelClass = useMemo(() => {
    return [
      ...
      isRndMode
        ? "bg-gradient-to-b from-amber-50/70 via-white/85 to-emerald-100/60 border-[#c8a97e]/50 text-slate-900 dark:from-slate-950/90 dark:via-neutral-950/40 dark:to-slate-950/70 dark:border-[#c8a97e]/30 dark:text-white/80"
  ...
  const handleUrlSubmit = async () => {
    ...
    try {
      ...
      // Backend returns {recipe: {...}, source: "..."} — unwrap the recipe object
      const recipeData = data?.recipe || data;
      await onRecipeImport?.(recipeData);
      setRecipeUrl("");
    } catch (error: any) {
  ...
  const renderRecipeTools = () => (
    <>
      <div className="border-b border-gray-300/50 p-4 pt-5 dark:border-slate-700/60">
        ...
        <div className="mt-4">
          <label htmlFor="recipe-url" className="mb-1 block text-sm font-medium text-slate-700 dark:text-slate-200">Recipe URL</label>
          <div className="flex gap-1">
            <input
              id="recipe-url"
              name="recipe_url"
              className="flex-1 rounded border border-gray-400/50 bg-gray-100/50 p-2 text-sm text-slate-700 placeholder-gray-400 shadow-sm transition-colors focus:bg-white focus:outline-none dark:border-slate-700/60 dark:bg-slate-900/60 dark:text-white/80 dark:placeholder-slate-500"
              value={recipeUrl}
            ...
            <button
              ...
              className="rounded border border-gray-400/50 bg-white/70 px-3 py-2 text-sm text-slate-600 transition hover:bg-white/90 dark:border-slate-700/60 dark:bg-slate-900/60 dark:text-[#c8a97e]/80 dark:hover:bg-slate-900/80"
            ...
            <button
              ...
              className={`flex items-center justify-center rounded px-3 py-2 text-sm text-white transition ${
                isImporting
                  ...
                  : "bg-blue-500 hover:bg-blue-600 dark:bg-amber-500 dark:hover:bg-[#c8a97e]-400"
              }`}
            >
              {isImporting ? (
      ...
      <div className="flex-1 space-y-4 overflow-y-auto p-4 scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-gray-200 dark:scrollbar-thumb-slate-600 dark:scrollbar-track-slate-800">
        <div>
          <label htmlFor="recipe-image-upload" className="mb-1 block text-sm font-medium text-slate-700 dark:text-slate-200">Recipe Image</label>
          <input
            ...
            className="w-full rounded border border-gray-400/50 bg-gray-100/50 p-2 text-xs text-slate-600 transition focus:bg-white focus:outline-none dark:border-slate-700/60 dark:bg-slate-900/60 dark:text-white/80"
          />
          {image && (
            <div className="mt-2">
              ...
              <button
                type="button"
                className="mt-2 flex w-full items-center justify-center gap-2 rounded bg-blue-500 px-3 py-2 text-xs font-medium text-white transition hover:bg-blue-600 dark:bg-amber-500 dark:hover:bg-[#c8a97e]-400"
                onClick={() => {
                  try {
          ...
          <AccordionItem value="status" data-accordion-section="status">
            ...
            <AccordionContent>
              <select
                ...
                className="w-full rounded border border-gray-400/50 bg-gray-100/50 p-2 text-sm text-slate-700 transition focus:bg-white focus:outline-none dark:border-slate-700/60 dark:bg-slate-900/60 dark:text-white/80"
              >
              ...
              <div className="space-y-3">
                <div className="grid grid-cols-2 gap-2">
                  {recipeAccessList.map((item) => (
                    <label key={item} className="flex items-center gap-2 text-xs" htmlFor={`access-${item.toLowerCase()}`}>
                      <input
                        id={`access-${item.toLowerCase()}`}
                        name={`access_${item.toLowerCase()}`}
                        type="checkbox"
                        className="scale-75"
                ...
                <div className="border-t border-gray-400/30 dark:border-slate-700/40 pt-3">
                  <label className="flex items-start gap-2" htmlFor="global-recipe-opt">
                    <input
                      id="global-recipe-opt"
                      name="global_recipe_opt"
                      type="checkbox"
            ...
            <AccordionContent>
              <div className="grid grid-cols-2 gap-2">
                {axisOptions("allergens").map((option) => (
                  <label key={option.slug} className="flex items-center gap-2 text-xs" htmlFor={`allergen-${option.slug}`}>
                    <input
                      id={`allergen-${option.slug}`}
                      name={`allergen_${option.slug}`}
                      type="checkbox"
                      className="scale-75"
            ...
            <AccordionContent>
              <div className="grid grid-cols-2 gap-2">
                {axisOptions("diets").map((option) => (
                  <label key={option.slug} className="flex items-center gap-2 text-xs" htmlFor={`diet-${option.slug}`}>
                    <input
                      id={`diet-${option.slug}`}
                      name={`diet_${option.slug}`}
                      type="checkbox"
                      className="scale-75"
            ...
            <AccordionContent>
              <div className="max-h-40 overflow-auto rounded border border-dashed border-gray-300/60 p-2 dark:border-slate-700/60">
                {axisOptions("course").map((option) => (
                  <label key={option.slug} className="flex items-center gap-2 text-xs" htmlFor={`course-${option.slug}`}>
                    <input
                      id={`course-${option.slug}`}
                      name={`course_${option.slug}`}
                      type="checkbox"
                      className="scale-75"
            ...
            <AccordionContent>
              <div className="max-h-40 overflow-auto rounded border border-dashed border-gray-300/60 p-2 dark:border-slate-700/60">
                {axisOptions("technique").map((option) => (
                  <label key={option.slug} className="flex items-center gap-2 text-xs" htmlFor={`technique-${option.slug}`}>
                    <input
                      id={`technique-${option.slug}`}
                      name={`technique_${option.slug}`}
                      type="checkbox"
                      className="scale-75"
            ...
            <AccordionContent>
              <div className="max-h-40 overflow-auto rounded border border-dashed border-gray-300/60 p-2 dark:border-slate-700/60">
                {axisOptions("equipment").map((option) => (
                  <label key={option.slug} className="flex items-center gap-2 text-xs" htmlFor={`equipment-${option.slug}`}>
                    <input
                      id={`equipment-${option.slug}`}
                      name={`equipment_${option.slug}`}
                      type="checkbox"
                      className="scale-75"
          ...
          <AccordionItem value="notes" data-accordion-section="notes">
            ...
            <AccordionContent>
              <textarea
                ...
                className="h-20 w-full rounded border border-gray-400/50 bg-gray-100/50 p-2 text-sm text-slate-700 transition focus:bg-white focus:outline-none dark:border-slate-700/60 dark:bg-slate-900/60 dark:text-white/80 dark:placeholder-slate-500"
              />
            </AccordionContent>
          </AccordionItem>
        </Accordion>
  ...
  const renderRndTools = () => (
    <>
      <div className="flex items-center justify-between border-b border-[#c8a97e]/50 p-4 dark:border-[#c8a97e]/25">
        <div className="space-y-1">
          <span className="text-[11px] font-semibold uppercase tracking-[0.35em] text-[#c8a97e] dark:text-[#c8a97e]/80/70">
            R&D signal deck
          </span>
          <h3 className="text-sm font-semibold tracking-tight text-[#c8a97e]/30 dark:text-white/80">
        ...
        <button
          type="button"
          onClick={() => onCloseLabs?.()}
          className="flex h-9 w-9 items-center justify-center rounded-full border border-[#c8a97e]/60 bg-[#c8a97e]/12 text-[#c8a97e]/30 transition hover:bg-[#c8a97e]/20 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#c8a97e]/60 focus-visible:ring-offset-2 focus-visible:ring-offset-amber-50 dark:border-[#c8a97e]/30 dark:bg-[#c8a97e]/15 dark:text-white/80"
          aria-label="Return to recipe tools"
        >
          <ArrowUpRight className="h-4 w-4" />
        </button>
      </div>
      <div className="flex-1 space-y-4 overflow-y-auto px-4 py-4 text-sm scrollbar-thin scrollbar-thumb-[#c8a97e]/60 scrollbar-track-white/40 dark:scrollbar-thumb-amber-500/50 dark:scrollbar-track-slate-900/50">
        {labStore ? (
          <>
            <section className="rounded-2xl border border-[#c8a97e]/40 bg-amber-50/70 p-4 shadow-inner dark:border-[#c8a97e]/25 dark:bg-slate-950/60">
              <header className="flex items-center justify-between text-[11px] uppercase tracking-[0.35em] text-[#b8976c] dark:text-[#c8a97e]/80/70">
                <span>Focus experiment</span>
                <span>{focusExperiment?.status ?? "—"}</span>
              </header>
              <h4 className="mt-2 text-base font-semibold tracking-tight text-[#c8a97e]/30 dark:text-white/80">
                {focusExperiment?.title ?? "No experiment selected"}
              </h4>
              <p className="mt-2 text-xs leading-relaxed text-[#c8a97e]/40/80 dark:text-white/80/80">
                {focusExperiment?.notes ?? "Select an experiment to view notes and context."}
              </p>
              {focusExperiment ? (
                <div className="mt-3 flex flex-wrap gap-2 text-[11px] uppercase tracking-[0.3em] text-[#b8976c]/80 dark:text-[#c8a97e]/80/80">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {(focusExperiment?.tags ?? []).map((tag) => (
                  <span
                    key={tag}
                    className="rounded-full bg-[#c8a97e]/12 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.35em] text-[#b8976c] dark:bg-[#c8a97e]/15 dark:text-[#c8a97e]/80"
                  >
                    {tag}
                  </span>
                ))}
            ...
            <section className="space-y-3 rounded-2xl border border-[#c8a97e]/30 bg-white/80 p-4 shadow-sm dark:border-[#c8a97e]/25 dark:bg-slate-950/50">
              <header className="flex items-center justify-between text-[11px] font-semibold uppercase tracking-[0.35em] text-[#b8976c] dark:text-[#c8a97e]/80/70">
              ...
              <div className="space-y-2">
                  ...
                  return (
                    <button
                      ...
                      className={`w-full rounded-xl border px-3 py-2 text-left text-xs transition ${
                        isActive
                          ? "border-[#c8a97e] bg-[#c8a97e]/08 text-[#c8a97e]/30 dark:border-[#c8a97e]/70 dark:bg-[#c8a97e]/15 dark:text-white/80"
                          : "border-transparent bg-white/60 text-slate-600 hover:border-[#c8a97e]/70 hover:bg-[#c8a97e]/08 hover:text-[#c8a97e]/30 dark:bg-slate-950/40 dark:text-[#c8a97e]/80/70 dark:hover:border-[#c8a97e]/40 dark:hover:text-amber-50"
                      }`}
                    >
                      ...
                      <div className="mt-2 flex flex-wrap gap-2 text-[10px] uppercase tracking-[0.35em] text-[#b8976c]/80 dark:text-[#c8a97e]/80/70">
                ...
                {discoveryQueue.length === 0 && (
                  <div className="rounded-xl border border-dashed border-[#c8a97e]/60 bg-white/70 px-3 py-2 text-xs text-[#b8976c] dark:border-[#c8a97e]/30 dark:bg-slate-950/50 dark:text-[#c8a97e]/80/70">
            ...
            <section className="space-y-3 rounded-2xl border border-[#c8a97e]/30 bg-white/80 p-4 shadow-sm dark:border-[#c8a97e]/25 dark:bg-slate-950/50">
              <header className="text-[11px] font-semibold uppercase tracking-[0.35em] text-[#b8976c] dark:text-[#c8a97e]/80/70">
                Backlog triggers
              </header>
              <div className="space-y-2">
                {backlog.map((task) => (
                  <div
                    key={task.id}
                    className="flex items-start justify-between gap-3 rounded-xl border border-[#c8a97e]/30 bg-white/70 px-3 py-2 text-xs dark:border-[#c8a97e]/25 dark:bg-slate-950/60"
                  >
                    <div>
                      <div className="font-semibold text-[#c8a97e]/30 dark:text-white/80">{task.label}</div>
                      <div className="text-[10px] uppercase tracking-[0.35em] text-[#b8976c]/80 dark:text-[#c8a97e]/80/70">
                        Owner: {task.owner}
                      </div>
                    </div>
                    <div className="text-right text-[10px] uppercase tracking-[0.35em] text-[#b8976c]/80 dark:text-[#c8a97e]/80/70">
                ...
                {backlog.length === 0 && (
                  <div className="rounded-xl border border-dashed border-[#c8a97e]/60 bg-white/70 px-3 py-2 text-xs text-[#b8976c] dark:border-[#c8a97e]/30 dark:bg-slate-950/60 dark:text-[#c8a97e]/80/70">
            ...
            <section className="space-y-2 rounded-2xl border border-[#c8a97e]/30 bg-white/80 p-4 text-xs shadow-sm dark:border-[#c8a97e]/25 dark:bg-slate-950/50 dark:text-white/80">
              <header className="text-[11px] font-semibold uppercase tracking-[0.35em] text-[#b8976c] dark:text-[#c8a97e]/80/70">
                Insights feed
              </header>
              <div className="space-y-2">
                {insights.map((item, index) => (
                  <div
                    ...
                    className="rounded-xl border border-[#c8a97e]/30 bg-white/70 px-3 py-2 shadow-sm dark:border-[#c8a97e]/25 dark:bg-slate-950/60"
                  >
                    <div className="text-xs font-semibold text-[#c8a97e]/30 dark:text-white/80">{item.headline}</div>
                    <p className="text-[11px] leading-relaxed text-[#c8a97e]/40/80 dark:text-[#c8a97e]/80/80">{item.detail}</p>
                    {item.metric ? (
          ...
          <div className="flex h-full items-center justify-center rounded-2xl border border-dashed border-[#c8a97e]/60 bg-amber-50/60 p-6 text-center text-sm text-[#b8976c] dark:border-[#c8a97e]/25 dark:bg-slate-950/60 dark:text-[#c8a97e]/80/70">
