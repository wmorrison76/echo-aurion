...
type ReferenceImagePayload = { dataUrl: string; name?: string };
...
export const handleImageGenerate: RequestHandler = async (req, res) => {
  try {
    ...
    const effectiveSize = ALLOWED_SIZES.has(String(size))
      ? String(size)
      : DEFAULT_SIZE;
    const effectiveQuality = ALLOWED_QUALITIES.has(String(quality))
      ? String(quality)
      : DEFAULT_QUALITY;
    const apiKey =
    ...
    if (!referenceImage?.dataUrl) {
      ...
      if (cached) {
        res.json({ ok: true, ...cached, cacheHit: true });
        return;
      }
    }
    ...
    if (referenceImage?.dataUrl) {
      ...
      result = await callOpenAIForGeneration(
        apiKey,
        trimmedPrompt,
        effectiveSize,
        effectiveQuality,
