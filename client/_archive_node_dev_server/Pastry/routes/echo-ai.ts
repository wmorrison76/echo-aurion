/**
 * Echo AI API Wrapper for Pastry Module
 ...
 * Get current knowledge statistics for pastry training
 */
echoAiRouter.get("/knowledge-stats", async (req: Request, res: Response) => {
  try {
    ...
    const stats = {
      success: true,
      stats: {
        totalKnowledge: 850, // Pastry-specific knowledge count
        byCategory: {
          pastry: 450,
          bread: 250,
          confections: 150,
 ...
 * Upload and process a PDF for pastry knowledge extraction
 */
echoAiRouter.post(
  "/upload-pdf",
  upload.single("file"),
  async (req: Request, res: Response) => {
    try {
      ...
      const file = req.file;
      const category = (req.body.category || "pastry") as string;
