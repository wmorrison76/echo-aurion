...
scheduleRouter.post("/upsert", async (req, res) => {
  ...
  if (!outlet || !weekStartISO || !data)
    return res
      .status(400)
      .json({ error: "outlet, weekStartISO, data required" });
  const week = weekStartISO.slice(0, 10);
  const row = { outlet, week_start: week, data };
  const { error } = await supa
    .from("schedules")
    .upsert(row, { onConflict: "outlet,week_start" });
...
scheduleRouter.get("/get", async (req, res) => {
  ...
  if (!outlet || !weekStartISO)
    return res.status(400).json({ error: "outlet and weekStartISO required" });
  const week = weekStartISO.slice(0, 10);
