import EchoOrb, { type EchoOrbProps, orbHostClass } from "@/components/echo/EchoOrb";
...
function Screensaver({
  ...
  useEffect(() => {
    if (!open) return;
    let el = document.getElementById("orb-screensaver") as HTMLDivElement | null;
    if (!el) {
      el = document.createElement("div");
      el.id = "orb-screensaver";
      Object.assign(el.style, { position: "fixed", inset: "0", zIndex: "9998" });
      document.body.appendChild(el);
    }
    setHost(el);
    return () => {
  ...
  return createPortal(
    <div className="h-full w-full bg-black" onClick={onClose} onKeyDown={onClose} tabIndex={0}>
...
export default function EchoControlsPage() {
  ...
  const [pattern, setPattern] = useState<"classic" | "waves" | "galaxy" | "fractal" | "tessellation">(
  ...
  return (
    <main className="container py-6">
          ...
          <Card className="border-none bg-gradient-to-br from-background/85 via-background/70 to-background/60 shadow-xl ring-1 ring-primary/10">
            <CardHeader className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
              <div>
                ...
                <CardDescription>
                  Live orb preview with internal opposing rings. Use the controls
                  below.
                </CardDescription>
              </div>
            ...
            <CardContent className="space-y-6">
              <div className="mx-auto w-full max-w-[520px]">
                <div className={`${orbHostClass} aspect-square`}>
                  <EchoOrb bare className="h-full w-full" {...sharedOrbProps} />
                </div>
              </div>
                ...
                <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3 text-xs">
                  <div className="flex flex-col gap-1">
                    ...
                    <select
                      value={chaosMode}
                      onChange={(event) =>
                        setChaosMode(event.target.value === "rossler" ? "rossler" : "lorenz")
                      }
                      className="rounded-md border bg-background px-2 py-1 text-xs"
                    >
                ...
                <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3 text-xs">
                  <div className="flex flex-col gap-1">
                    ...
                    <select
                      ...
                      onChange={(event) =>
                        setTessellationType(event.target.value === "tri" ? "tri" : "hex")
                      }
                      className="rounded-md border bg-background px-2 py-1 text-xs"
                    >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <input
                      ...
                      onChange={(event) => setTessellationColorA(event.target.value)}
                      className="h-6 w-10 cursor-pointer rounded-md border bg-transparent"
                    />
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    <input
                      ...
                      onChange={(event) => setTessellationColorB(event.target.value)}
                      className="h-6 w-10 cursor-pointer rounded-md border bg-transparent"
                    />
                  </div>
                </div>
      ...
      <BackgroundLayer enabled={bgEnabled} url={bgUrl} orbProps={sharedOrbProps} />
    </main>
  );
}
