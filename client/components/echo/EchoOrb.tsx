...
export const orbHostClass =
  "relative flex items-center justify-center overflow-hidden rounded-full bg-[radial-gradient(circle_at_center,_rgba(12,18,32,0.88)_0%,_rgba(5,10,21,0.92)_55%,_rgba(2,6,14,0.72)_100%)] shadow-[0_0_32px_rgba(56,189,248,0.35)] backdrop-blur-sm border-0 outline-none focus:outline-none ring-0 focus:ring-0 !border-none !outline-none";
  ...
  const mat = useMemo(
    () =>
      new THREE.PointsMaterial({
        size: isDark ? safeParticleSize : safeParticleSize * 1.3,
        color,
        transparent: true,
        opacity: 0.98,
        depthWrite: false,
...
function TessellationField({
  ...
  const data = useMemo(() => {
    ...
    const triHeight = Math.sqrt(3) * 0.5 * scale;
    let ptr = 0;
    for (let row = 0; row < safeRows; row++) {
      for (let col = 0; col < safeCols; col++) {
        ...
        let pz = 0;
        if (type === "tri") {
          const offset = row % 2 === 0 ? 0 : scale * 0.5;
          px = (col - halfCols) * scale + offset;
          pz = (row - halfRows) * triHeight * 0.96;
        } else {
          const offset = row % 2 === 0 ? 0 : scale * 0.5;
          px = (col - halfCols) * scale + offset;
          pz = (row - halfRows) * scale * 0.86;
        }
        positions[ptr] = px;
        positions[ptr + 1] = 0;
        positions[ptr + 2] = pz;
...
function WaveField({
  ...
  const geom = useMemo(()=>{
    ...
    let k=0; const w=(cols-1)*spacing*0.5; const h=(rows-1)*spacing*0.5;
  ...
  const mat = useMemo(()=> new THREE.PointsMaterial({ size: 0.015, color, transparent:true, opacity:0.95, blending:THREE.AdditiveBlending, depthWrite:false, toneMapped:false }),[color]);
  useFrame(({clock})=>{
    if(!ref.current) return;
    const t = clock.getElapsedTime()*speed;
    const arr = (ref.current.geometry.getAttribute('position') as THREE.BufferAttribute).array as Float32Array;
    let k=0; for(let j=0;j<rows;j++){
      for(let i=0;i<cols;i++){
        const x = i*spacing; const z = j*spacing;
        arr[k+1] = Math.sin(x*0.4 + t) * Math.cos(z*0.35 + t*0.7) * amp; // y
        k+=3;
      }
    }
...
export default function EchoOrb({
  ...
  return (
    <div className={composed}>
      <ErrorBoundary
        fallback={
          <div className="flex h-full w-full items-center justify-center rounded-full bg-slate-900/50 text-xs text-slate-500">
            Orb Error
          </div>
        }
      >
