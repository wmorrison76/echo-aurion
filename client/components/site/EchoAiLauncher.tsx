...
import { escalateToEchoCoder } from "@/components/echo/EchoOrbEscalation";
...
const ICON_URL =
  "data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='none' stroke='currentColor' stroke-width='2'%3E%3Cpath d='M12 2a2 2 0 012 2c0 .74-.4 1.39-1 1.73V7h1a7 7 0 017 7h1a1 1 0 010 2h-1a7 7 0 01-7-7h-1V6.73c.6-.34 1-.99 1-1.73a2 2 0 012-2z'/%3E%3Ccircle cx='12' cy='17' r='3'/%3E%3C/svg%3E";
...
const fixedLauncherChromeClass = "bg-transparent backdrop-blur-xl shadow-none";
...
export default function EchoAiLauncher({
  ...
  const [reduceMotion, setReduceMotion] = React.useState(() => {
    try {
      const saved = safeGet("system-preferences");
      if (saved) {
        const prefs = JSON.parse(saved);
        return prefs.accessibility?.reduceMotion || false;
      }
    } catch {
      return false;
    }
  ...
  const [panelOpacity, setPanelOpacity] = React.useState<number>(() => {
    try {
      // If reduce motion is enabled, set opacity to 1 (fully opaque, no transparency)
      ...
      const stored = parseFloat(safeGet("echo.popup.opacity") || "0.82");
  ...
  // Listen for reduce-motion changes and update opacity
  React.useEffect(() => {
    const handlePreferencesChange = (e: Event) => {
      try {
        ...
        if (
          ...
          const newReduceMotion = detail.value || false;
          ...
          if (saved) {
            const prefs = JSON.parse(saved);
            const newReduceMotion = prefs.accessibility?.reduceMotion || false;
            setReduceMotion(newReduceMotion);
            if (newReduceMotion) {
              setPanelOpacity(1);
            }
  ...
  const [avatarChatActive, setAvatarChatActive] = React.useState(false);
  ...
  // Listen for avatar chat state changes to hide/show orb
  React.useEffect(() => {
    const handleAvatarChatState = (e: any) => {
      const isActive = e.detail?.active || false;
      setAvatarChatActive(isActive);
    };

    // Check initial state from localStorage
    const savedState = (() => {
      try {
        return window.localStorage.getItem("avatar.chat.open") === "1";
      } catch {
        return false;
      }
    })();
    setAvatarChatActive(savedState);

    window.addEventListener("avatar-chat-state", handleAvatarChatState as any);
    return () =>
      window.removeEventListener(
        "avatar-chat-state",
        handleAvatarChatState as any,
      );
  }, []);
  ...
  React.useEffect(() => {
    ...
    if (lastMessage && lastMessage.text.toLowerCase().includes("echo")) {
      setIsHidden(false);
    }
  }, [history, isHidden]);
  React.useEffect(() => {
  ...
  async function handleCommand(prompt: string) {
    ...
    // navigation
    if (/\b(go to|open|navigate|mostrar|abrir|show)\b/i.test(m)) {
      if (/\borb\b/i.test(m)) {
      ...
      if (/\b(culinary|recipes?|recipe.*pro)\b/i.test(m)) {
        window.dispatchEvent(
          new CustomEvent("open-panel", { detail: { id: "culinary" } }),
        );
        return "Opening Culinary module for you!";
      }
      if (/\b(inventory|stock)\b/i.test(m)) {
        window.dispatchEvent(
          new CustomEvent("open-panel", { detail: { id: "inventory" } }),
        );
        return "Opening Inventory module for you!";
      }
      if (/\b(schedule|scheduling|labor)\b/i.test(m)) {
        window.dispatchEvent(
          new CustomEvent("open-panel", { detail: { id: "schedule" } }),
        );
        return "Opening Schedule module for you!";
      }
      if (/\b(purchasing|receiving|purchasingreceiving)\b/i.test(m)) {
        window.dispatchEvent(
          new CustomEvent("open-panel", {
            detail: { id: "purchasingreceiving" },
          }),
        );
        return "Opening Purchasing & Receiving module for you!";
      }
      if (/\b(maestro|bqt|banquet)\b/i.test(m)) {
        window.dispatchEvent(
          new CustomEvent("open-panel", { detail: { id: "maestrobqt" } }),
        );
        return "Opening Maestro BQT module for you!";
      }
      if (/\b(settings?|preferences?|config)\b/i.test(m)) {
        window.dispatchEvent(
          new CustomEvent("open-panel", { detail: { id: "settings" } }),
        );
        return "Opening Settings for you!";
      }
      if (/\b(dashboard|home)\b/i.test(m)) {
        window.dispatchEvent(
    ...
    if (/snapshot|backup/i.test(m)) {
      try {
        const r = await fetch("/api/zaro/snapshot", { method: "POST" });
    ...
    if (/integrity|check/i.test(m)) {
      try {
        const r = await fetch("/api/zaro/integrity", { method: "POST" });
        const j = await r.json();
        return r.ok || j.ok
    ...
    if (/restore/i.test(m)) {
      try {
        const r = await fetch("/api/zaro/restore", { method: "POST" });
        const j = await r.json();
        if (r.ok && j.ok) {
  ...
  function isAbortError(error: unknown, controller?: AbortController | null) {
    // AbortError is no longer generated - all timeouts use Promise.race with regular Errors
    try {
      if (!error) {
        return false;
      }
      if (typeof error === "string") {
        ...
        // Check for timeout messages from Promise.race
  ...
  async function safeFetch(input: RequestInfo | URL, init?: RequestInit) {
    ...
    while (attempt <= maxRetries) {
      try {
        ...
        // Ensure we have a fresh response with an unread body stream
        // This prevents "body stream already read" errors by returning a properly cloned response
        return res.clone();
      } catch (error: any) {
  ...
  async function callPlanner(prompt: string) {
    ...
    plannerAbortRef.current = { signal: { aborted: false } } as any;
    const requestId = plannerAbortRef.current;
    const isActive = () => plannerAbortRef.current === requestId;
    const ensureActive = () => isActive();
    try {
      const echoResponse = await safeFetch("/api/echo", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      const planResponse = await safeFetch("/api/plan", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          prompt: message,
      ...
      if (plannerAbortRef.current === requestId) {
  ...
  async function submitPrompt(
    ...
    try {
      ...
      // SECURITY: Do NOT execute arbitrary code from AI responses
      // Code execution has been disabled due to security risks (XSS, code injection)
      // If code examples need to be displayed, show them as text/syntax-highlighted content only
      ...
      while ((match = codeBlockRegex.exec(response)) !== null) {
        codeBlockCount++;
        detectedCodeBlocks.push(match[1]);
      }
      if (codeBlockCount > 0) {
        console.warn(
          `[Echo] SECURITY: ${codeBlockCount} code block(s) detected but NOT executed for security reasons. Code execution from AI responses is disabled.`,
          detectedCodeBlocks,
        );
      }
  ...
  const popupPlacementClass =
    variant === "header"
      ...
      : isStudio
        ...
        : "absolute top-[calc(100%+70px)] right-0";
  ...
  // Listen for Echo V5 autonomous actions and inject into chat
  React.useEffect(() => {
    const handleEchoAction = (event: Event) => {
      const customEvent = event as CustomEvent;
      const action = customEvent.detail;

      if (action?.text) {
        // Inject Echo's proactive message into chat history
        setHistory((h) => [
          ...h,
          {
            role: "ai",
        ...
        // Auto-expand Echo launcher when she speaks
        if (!open) {
          setOpen(true);
        }

        console.log("[EchoAiLauncher] Received proactive action:", action);
      }
    };

    window.addEventListener("echo-action", handleEchoAction);
    return () => window.removeEventListener("echo-action", handleEchoAction);
  }, [open]);

  // Hide orb when avatar chat is active
  if (avatarChatActive && variant !== "header") {
    return null;
  }
  ...
  return (
    <div
      ...
      className={
        variant === "header"
          ? "relative z-[50000]"
          : `${isStudio ? "fixed right-4 bottom-4" : "fixed right-4 top-4"} z-[50000]`
      }
      onMouseEnter={() => {
        setHovering(true);
        if (!pinned) {
    ...
    >
      <button
        ...
        } ${pinned ? "ring-2 ring-primary/60" : ""} border-0 outline-none focus:outline-none`}
      ...
      >
        <div
          className={`${orbHostClass} ${isLightMode ? "![box-shadow:none] !shadow-none" : ""} border-0 outline-none focus:outline-none`}
          style={orbFrameStyle}
        ...
        >
          {fallback ? (
            ...
            <div className="h-full w-full rounded-full bg-gradient-to-br from-primary/60 to-primary/30 backdrop-blur-xl" />
          ) : (
            <EchoOrb
              key={orbKey}
              bare
      ...
      {variant === "header" ? (
        <span
          className={`pointer-events-none absolute right-full top-1/2 mr-3 -translate-y-1/2 select-none rounded-full bg-[hsl(var(--background)/0.42)] px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.28em] text-primary shadow-[0_0_28px_rgba(56,189,248,0.45)] backdrop-blur-2xl transition-opacity duration-300 ${
      ...
      {open && !isHidden && (
        <div className={`pointer-events-auto ${popupPlacementClass} z-[50001]`}>
          <div
            style={panelStyle}
          ...
          >
            ...
            <header className="relative z-[1] flex flex-col gap-3">
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-2">
                  <button
                    type="button"
                    onClick={() => togglePinnedState()}
                    className={`${controlChipBaseClass} ${pinned ? activeControlClass : ""}`}
                    aria-pressed={pinned}
                  >
                    {pinned ? "Undock" : "Dock"}
                  </button>
                  ...
                  <button
                    ...
                    className={controlChipBaseClass}
                    title="Hide chat (say 'Echo' to unhide)"
                  >
                    Hide
                  </button>
                </div>
              ...
              <div className="flex flex-col gap-1 rounded-2xl bg-[hsl(var(--background)/0.38)] px-4 py-3 text-[11px] text-[hsl(var(--foreground)/0.72)] shadow-[inset_0_0_24px_rgba(8,14,27,0.45)]">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <span className="text-[10px] text-[hsl(var(--foreground)/0.6)]">
                    {Math.round(avgSentiment * 100)}% sentiment
                  </span>
                </div>
                <span className="text-[10px] text-[hsl(var(--foreground)/0.55)]">
                  {persona.tone}
                </span>
                {Object.keys(contextSummary).length ? (
                  <span className="text-[10px] text-[hsl(var(--foreground)/0.45)]">
                    Context:{" "}
            ...
            >
              {history.length === 0 ? (
                <div
                  className={`${messageBubbleBaseClass} ${aiMessageBubbleClass}`}
                  style={bubbleStyles.ai}
                >
                  <p className="text-[12px] font-medium text-primary/90">
                    {t("echo.greeting")}
                  </p>
                  <p className="text-[12px] text-[hsl(var(--primary-foreground)/0.75)]">
                    {t("echo.intro")}
                  </p>
                </div>
              ) : (
                history.map((m, i) => (
                  <div
                    key={`${m.role}-${i}`}
                    className={`${messageBubbleBaseClass} ${
                      m.role === "ai"
                        ? aiMessageBubbleClass
                        : `${userMessageBubbleClass} ml-auto text-right`
                    }`}
                    style={
                      m.role === "ai" ? bubbleStyles.ai : bubbleStyles.user
                    }
                  >
                    {m.text}
                  </div>
              ...
              {pendingReply ? (
                ...
                >
                  <span className="flex items-center gap-2">
                    <span className="relative inline-flex h-2 w-8 justify-between">
                      <span className="h-2 w-2 animate-bounce rounded-full bg-primary/90 [animation-delay:-0.2s]" />
                      <span className="h-2 w-2 animate-bounce rounded-full bg-primary/75" />
                      <span className="h-2 w-2 animate-bounce rounded-full bg-primary/60 [animation-delay:0.2s]" />
                    </span>
                    <span className="text-[11px] uppercase tracking-[0.28em] text-primary/80">
                      {t("echo.thinking")}
                    </span>
                  </span>
                </div>
            ...
            <div className="relative z-[1] flex min-h-0 flex-col gap-4">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className={`${controlButtonClass} ${
                      value.trim()
                        ? "bg-primary/80 text-primary-foreground shadow-[0_0_36px_hsl(var(--primary)/0.55)]"
                  ...
                  <button
                    type="button"
                    onClick={toggleMic}
                    className={`${controlButtonClass} ${shouldListen ? activeControlClass : ""}`}
                    aria-pressed={shouldListen}
                    title={
                      shouldListen
                        ? t("echo.muteMicrophone")
                        : t("echo.enableMicrophone")
                    }
                  >
                    {shouldListen
                      ? listening
              ...
              <div className="flex flex-wrap items-center gap-2">
                <select
                  value={persona.mode}
                  onChange={(e) => {
                    const next = ai3.personas.setPersona(
                      e.target.value as PersonaProfile["mode"],
                    );
                    setPersona(next);
                  }}
                  className={`${controlSelectClass} min-w-[128px]`}
                  title={t("echo.selectPersona")}
                >
                ...
                <button
                  ...
                  className={`${controlChipBaseClass} ${voiceOn ? activeControlClass : ""}`}
                  aria-pressed={voiceOn}
                  title={t("echo.toggleVoiceReplies")}
                >
                  {t("echo.voiceReply")}
                </button>
                <button
                  type="button"
                  onClick={() => setUsePremium((prev) => !prev)}
                  className={`${controlChipBaseClass} ${usePremium ? activeControlClass : ""}`}
                  aria-pressed={usePremium}
                  title={t("echo.premium")}
                >
                  {t("echo.premium")}
                </button>
                {usePremium && premiumVoices ? (
                  <select
                    ...
                    className={`${controlSelectClass} min-w-[140px]`}
                    title={t("echo.selectPremiumVoice")}
                  >
                    {premiumVoices.map((v) => (
                ...
                <div className="flex flex-1 min-w-[160px] items-center gap-3 rounded-full bg-[hsl(var(--background)/0.32)] px-3 py-2 shadow-[inset_0_0_22px_rgba(8,14,27,0.45)]">
                  <span
                    className={`${subtleLabelClass} text-[hsl(var(--foreground)/0.7)]`}
                  >
                    {t("echo.auraDensity")}
                  </span>
                  <input
                    ...
                    onChange={(e) => updatePanelOpacity(e.target.value)}
                    onPointerUp={announceAuraChange}
                    onTouchEnd={announceAuraChange}
                    onKeyUp={(e) => {
                      if (
                        [
                          "Enter",
                    ...
                    className="h-1 flex-1 appearance-none rounded-full bg-[hsl(var(--foreground)/0.18)]"
                    style={{ accentColor: "hsl(var(--primary))" }}
                    aria-label={t("echo.adjustChatOpacity")}
                  />
                  <span className="text-[11px] font-medium text-[hsl(var(--foreground)/0.72)]">
                ...
                <button
                  ...
                  onClick={async () => {
                    try {
                      if (lastTts?.blob) {
                        const name = `echo-${speechLocale || "voice"}-${Date.now()}.mp3`;
                        const link = document.createElement("a");
                        link.href =
                          lastTts.url || URL.createObjectURL(lastTts.blob);
                        link.download = name;
                      ...
                      const last =
                        [...history].reverse().find((h) => h.role === "ai")
                          ?.text ||
                        value ||
                        "";
                      if (!last || !premiumVoiceId) return;
                      const r = await fetch(`/api/tts/speak?download=1`, {
                        method: "POST",
                        headers: { "Content-Type": "application/json" },
                        body: JSON.stringify({
                          text: last,
                      ...
                      if (r.ok) {
                        ...
                        link.download = `echo-${speechLocale || "voice"}-${Date.now()}.mp3`;
                        document.body.appendChild(link);
                        link.click();
                        setTimeout(() => link.remove(), 0);
                      }
                  ...
                  className={`${controlChipBaseClass} flex items-center gap-1`}
                  title={t("echo.downloadLastVoice")}
                >
                  ⬇️ <span>{t("echo.save")}</span>
                </button>
              </div>
            </div>
          </div>
