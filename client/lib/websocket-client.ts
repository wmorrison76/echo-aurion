...
class WebSocketClient {
      ...
      try {
        ...
        this.ws.onerror = (event: Event) => {
          ...
          logger.error("WebSocket connection error", {
            ...
            readyStateLabel: ws
              ? this.getReadyStateLabel(ws.readyState)
              : "unknown",
  ...
  private attemptReconnect() {
    ...
    setTimeout(() => {
      this.connect().catch((error) => {
        logger.error("WebSocket reconnection attempt failed", {
          attempt: this.reconnectAttempts,
          error: error instanceof Error ? error.message : String(error),
          url: this.url,
        });
  ...
  private handleMessage(message: WebSocketMessage) {
    ...
    switch (type) {
      ...
      case "unsubscribed":
        logger.debug("Successfully unsubscribed", { channel });
        break;
  ...
  subscribe(
    ...
    if (!this.ws || this.ws.readyState !== WebSocket.OPEN) {
      logger.warn("WebSocket not connected, cannot subscribe", { channel });
  ...
  off(channel: string, listener: ChannelListener): void {
    ...
    if (listeners) {
      listeners.forEach((listener) => {
        try {
          ...
          logger.error("Error in channel listener", {
            channel,
            error: error instanceof Error ? error.message : String(error),
          });
        }
...
export function initializeWebSocket(): Promise<WebSocketClient> {
