...
function restoreFromGit(filepath) {
  try {
    ...
    // Try to find last commit where file was properly formatted
    try {
      ...
      for (const commit of commits) {
        if (!commit) continue;
        try {
          const gitContent = execSync(`git show ${commit}:${filepath}`, {
            encoding: "utf8",
            stdio: "pipe",
          });
          ...
          if (gitLines > 25) {
            fs.writeFileSync(filepath, gitContent, "utf8");
            console.log(
              `  ✅ Restored from commit ${commit.substring(0, 8)}: ${filepath}`
            );
            return true;
          }
        } catch (e) {
