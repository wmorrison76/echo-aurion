...
export function estimateServings(tiers: TierSpec[], shape: "round" | "square" | "sheet") {
  if (shape === "round") return tiers.reduce((a, t) => a + servingsForRound(t.diameter || 0), 0);
  if (shape === "square") return tiers.reduce((a, t) => a + Math.round(servingsForRound(t.diameter || 0) * 1.25), 0);
  // sheet
  const sheetPreset = (w?: number, d?: number) => {
    const W = Math.round(w || 0); const D = Math.round(d || 0);
    const key = `${Math.min(W,D)}x${Math.max(W,D)}`;
    const map: Record<string, number> = { "9x13": 22, "13x18": 42, "18x26": 75 };
...
export function priceEstimate(design: DesignData): PricingBreakdown {
  ...
  const stand = design.stand === "gold" ? 25 : design.stand === "acrylic" ? 15 : 0;
  const complexity = Math.max(0, (design.tiers.length - 1) * 20 + (design.sliceAngle > 0 ? 10 : 0));
