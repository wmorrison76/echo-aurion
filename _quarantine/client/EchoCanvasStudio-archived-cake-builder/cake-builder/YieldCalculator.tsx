...
export default function YieldCalculator({
  ...
  const targetServings = useMemo(() => {
    return Math.ceil(targetGuests * servingsPerPerson * (1 + bufferPercentage / 100));
    ...
    <Card className="w-full">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          <div>
            <label className="text-sm font-semibold block mb-2">Per Person</label>
            <Input
              type="number"
              step="0.5"
              min="0.5"
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="p-3 bg-muted rounded">
            <div className="text-xs text-muted-foreground">Current Servings</div>
            <div className="text-2xl font-bold">{currentServings}</div>
            <div className="text-xs mt-1">From {currentTiers.length} tier(s)</div>
          </div>
          <div className={`p-3 rounded ${hasShortfall ? "bg-red-50" : "bg-green-50"}`}>
            <div className={`text-xs ${hasShortfall ? "text-red-600" : "text-green-600"}`}>
              Target Servings
            </div>
            <div className={`text-2xl font-bold ${hasShortfall ? "text-red-700" : "text-green-700"}`}>
              {targetServings}
            </div>
            {hasShortfall && (
        ...
        {hasShortfall && (
          <div className="bg-orange-50 border border-orange-200 p-3 rounded space-y-2">
            <h4 className="font-semibold text-orange-900 text-sm">Recommended Tier Configuration</h4>
