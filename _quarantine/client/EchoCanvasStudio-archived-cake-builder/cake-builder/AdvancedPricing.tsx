...
export default function AdvancedPricing({
  ...
  const calculateByYieldGoal = (targetYield: number) => {
    const servingCost = (costBreakdown.ingredientsCost + costBreakdown.laborCost) / Math.max(1, pricing.servings);
        ...
        <Tabs defaultValue="breakdown" className="w-full">
          ...
          <TabsContent value="breakdown" className="space-y-4 mt-4">
            <div className="space-y-3 text-sm">
              <div className="flex justify-between items-center p-3 bg-muted rounded">
                ...
                <div className="flex items-center gap-2">
                  <span className="font-semibold">${costBreakdown.ingredientsCost.toFixed(2)}</span>
                  <input
                    type="number"
                    value={costBreakdown.ingredientsCost}
                    onChange={(e) => handleCostChange("ingredientsCost", Number(e.target.value))}
              ...
              <div className="flex justify-between items-center p-3 bg-muted rounded">
                ...
                <div className="flex items-center gap-2">
                  <span className="font-semibold">${costBreakdown.laborCost.toFixed(2)}</span>
                  <input
                    type="number"
                    value={costBreakdown.laborCost}
                    onChange={(e) => handleCostChange("laborCost", Number(e.target.value))}
                    className="w-20 px-2 py-1 border rounded text-xs"
                  />
                </div>
              </div>
              ...
              <div className="flex justify-between items-center p-3 bg-muted rounded">
                ...
                <div className="flex items-center gap-2">
                  <span className="font-semibold">${costBreakdown.deliveryCost.toFixed(2)}</span>
                  <input
                    type="number"
                    value={costBreakdown.deliveryCost}
                    onChange={(e) => handleCostChange("deliveryCost", Number(e.target.value))}
                    className="w-20 px-2 py-1 border rounded text-xs"
                  />
                </div>
              </div>
              ...
              <div className="flex justify-between items-center p-3 bg-muted rounded">
                ...
                <div className="flex items-center gap-2">
                  <span className="font-semibold">${costBreakdown.equipmentCost.toFixed(2)}</span>
                  <input
                    type="number"
                    value={costBreakdown.equipmentCost}
                    onChange={(e) => handleCostChange("equipmentCost", Number(e.target.value))}
                    className="w-20 px-2 py-1 border rounded text-xs"
                  />
                </div>
              </div>
              ...
              <div className="border-t pt-2 flex justify-between items-center p-3 bg-primary/10 rounded font-semibold">
                ...
                <span>${(costBreakdown.ingredientsCost + costBreakdown.laborCost + costBreakdown.deliveryCost + costBreakdown.equipmentCost).toFixed(2)}</span>
              </div>
              ...
              <div className="flex justify-between items-center p-3 bg-green-50 rounded font-semibold text-green-900">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <input
                    ...
                    onChange={(e) => handleCostChange("markup", Number(e.target.value))}
                    className="w-20 px-2 py-1 border rounded text-xs"
                  />
                </div>
              </div>
            ...
            <div className="text-xs text-muted-foreground pt-2">
              Cost per serving: ${((costBreakdown.ingredientsCost + costBreakdown.laborCost) / Math.max(1, pricing.servings)).toFixed(2)}
            </div>
          </TabsContent>
