...
window.addEventListener("mousemove", onCursor); const onMsg = (data: any) => {
...
const onStorage = (ev: StorageEvent) => { if (ev.key === key && ev.newValue) try { onMsg(JSON.parse(ev.newValue).msg); } catch {} }; bc?.addEventListener("message", onBC);
window.addEventListener("storage", onStorage); // announce presence
send({ type:"hello", from: self.id, user: self }); return () => {
bc?.removeEventListener("message", onBC);
bc?.close();
window.removeEventListener("storage", onStorage);
window.removeEventListener("mousemove", onCursor);
};
}, [self]); const value = useMemo(() => ({ self, others }), [self, others]);
