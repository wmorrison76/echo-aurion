...
import { EmailTemplate, EmailIntegration } from "@shared/sales-pipeline-types";

// Mock data for demonstration
const mockEmailIntegrations: EmailIntegration[] = [
  {
    id: 'outlook-1',
    provider: 'outlook',
...
{{phone}} | {{email}}`,
    type: 'cold_outreach',
    variables: ['firstName', 'companyName', 'eventType', 'similarClient', 'guestCount', 'senderName', 'senderTitle', 'phone', 'email'],
    isActive: true,
...
{{venue}}`,
    type: 'proposal',
    variables: ['firstName', 'partnerName', 'venue', 'weddingDate', 'spaceType', 'capacity', 'suggestedDay', 'senderName', 'title'],
    isActive: true,
...
export default function EmailIntegration({ className }: EmailIntegrationProps) {
  const [integrations, setIntegrations] = useState<EmailIntegration[]>(mockEmailIntegrations);
  ...
  const getProviderColor = (provider: string) => {
    switch (provider) {
      case 'outlook': return 'bg-blue-100 text-blue-700 dark:bg-blue-900/20 dark:text-blue-300';
      case 'gmail': return 'bg-red-100 text-red-700 dark:bg-red-900/20 dark:text-red-300';
      case 'exchange': return 'bg-gray-100 text-gray-700 dark:bg-gray-900/20 dark:text-gray-300';
  ...
  const getTemplateTypeIcon = (type: string) => {
    switch (type) {
      case 'cold_outreach': return Target;
      case 'follow_up': return Clock;
      case 'proposal': return FileText;
      case 'meeting_request': return Calendar;
      case 'thank_you': return CheckCircle;
      case 'objection_handling': return Shield;
      default: return Mail;
    }
  };
          ...
          <Dialog open={isIntegrationDialogOpen} onOpenChange={setIsIntegrationDialogOpen}>
            ...
            <DialogContent className="glass-panel max-w-md">
              <DialogHeader>
                ...
                <DialogDescription>
                  Connect your email account to enable automated outreach
                </DialogDescription>
              </DialogHeader>
          ...
          <Dialog open={isTemplateDialogOpen} onOpenChange={setIsTemplateDialogOpen}>
            ...
            <DialogContent className="glass-panel max-w-4xl">
              <DialogHeader>
                ...
                <DialogDescription>
                  Create a reusable email template for your sales outreach
                </DialogDescription>
              </DialogHeader>
              <div className="grid gap-4 py-4">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {['firstName', 'lastName', 'companyName', 'eventType', 'eventDate', 'venue', 'guestCount'].map((variable) => (
                    ...
                    <div className="flex items-center space-x-2">
                      {integration.isConnected ? (
                        ...
                        <Button
                          size="sm"
                          className="apple-button"
                          onClick={() => handleConnectEmailProvider(integration.provider as 'gmail' | 'outlook' | 'yahoo' | 'exchange')}
                          disabled={isConnecting}
                        >
        ...
        <TabsContent value="templates" className="space-y-6 mt-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {templates.map((template) => {
              ...
              return (
                <Card key={template.id} className="glass-panel cursor-pointer hover:border-primary/50 transition-colors">
          ...
          {selectedTemplate && (
            <>
              <DialogHeader>
                ...
                <DialogDescription>
                  {selectedTemplate.type.replace('_', ' ')} template • Used {selectedTemplate.usageCount} times • {selectedTemplate.successRate}% success rate
                </DialogDescription>
              </DialogHeader>
