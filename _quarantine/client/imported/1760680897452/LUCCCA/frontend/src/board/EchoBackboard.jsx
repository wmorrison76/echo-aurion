...
/** EchoBackboard — ambient background, toast, and private stickies */
export default function EchoBackboard({
  ...
  const addNote = (partial = {}) => {
    ...
    const n = {
      id,
      x: partial.x ?? 120 + Math.floor(Math.random() * 60),
      y: partial.y ?? 110 + Math.floor(Math.random() * 50),
      w: partial.w ?? 260,
      h: partial.h ?? 170,
  ...
  const updateNote = (id, patch) => setNotes(v => v.map(n => (n.id === id ? { ...n, ...patch, updatedAt: Date.now() } : n)));
    ...
    const tick = () => {
      ...
      notes.forEach((n) => {
        if (n.dueAt && !n._fired && now >= n.dueAt) {
          window.dispatchEvent(new CustomEvent("echo-notify", { detail: { text: `⏰ ${n.title || "Reminder"}`, tone: "ok", ms: 5000 } }));
  ...
  useEffect(() => {
    ...
    window.addEventListener("echo-add-note", addFromEvent);
    window.addEventListener("backboard-add-sticky", addFromEvent);
    window.addEventListener("echo-assist", onAssist);
    return () => {
      window.removeEventListener("echo-notify", onNotify);
      window.removeEventListener("echo-add-note", addFromEvent);
      window.removeEventListener("backboard-add-sticky", addFromEvent);
  ...
  // read CSS variables driving the backboard
  const cv = (name, fallback) => getComputedStyle(document.documentElement).getPropertyValue(name).trim() || fallback;

  const backImage   = cv("--backboard-image", "");
  const backOpacity = parseFloat(cv("--backboard-opacity", "0.20") || "0.20");
  const backBlur    = cv("--backboard-blur", "2px");
  const backFit     = cv("--backboard-fit", "cover");
  const backPos     = cv("--backboard-pos", "center");
  const backGrad    = cv("--backboard-bg", "radial-gradient(1200px 520px at 76% 8%, rgba(22,224,255,.08), transparent 60%)");
  const backOverlay = cv("--backboard-overlay", "radial-gradient(1400px 540px at 78% 10%, rgba(22,224,255,.08), transparent 62%)");

  return (
    <div className="echo-backboard" style={{ position: "absolute", inset: 0, zIndex: 2, pointerEvents: "none" }} aria-hidden>
      {/* Base gradient + overlay */}
      <div style={{ position: "absolute", inset: 0, background: backGrad }} />
      <div style={{ position: "absolute", inset: 0, background: backOverlay, opacity: 1 }} />

      {/* Optional background image (obeys CSS vars) */}
      {backImage && (
        <div
          style={{
            position: "absolute", inset: 0, opacity: backOpacity,
            backgroundImage: `url(${backImage.replace(/(^"|"$)/g, "")})`,
            backgroundSize: backFit, backgroundPosition: backPos, backgroundRepeat: "no-repeat",
            filter: `blur(${backBlur})`,
          }}
        />
      )}

      {/* Outer frame / glow so panels read against light mode too */}
      <div
        style={{
          position: "absolute", inset: 0, borderRadius: 18,
          boxShadow: "0 28px 120px rgba(0,0,0,.45), 0 0 0 1px rgba(255,255,255,.06), inset 0 0 0 1px rgba(255,255,255,.04)",
          pointerEvents: "none",
        }}
      />

      {/* pulse wash when assistant responds */}
      <div
        style={{
          position: "absolute",
          inset: 0,
      ...
      {/* (optional) tiny orb cluster – hidden by default */}
      {showOrb && (
        <>
          <div
            role="button"
            title={`Ask Echo (${status})`}
            onClick={() => {
            ...
            style={{
              position: "absolute", top: 12, right: 14, width: 34, height: 34, borderRadius: 999,
              background: "rgba(10,16,28,.75)", border: "1px solid rgba(22,224,255,.38)",
              boxShadow: `${statusDot}, inset 0 0 0 1px rgba(255,255,255,.05)`,
              color: "#aef", display: "grid", placeItems: "center", pointerEvents: "auto",
              transform: visible ? "none" : "translateY(-6px)", opacity: visible ? 1 : 0.25,
              transition: "opacity .25s ease, transform .25s ease", cursor: "pointer",
            }}
          >
            <svg width="15" height="15" viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="4" fill="currentColor" /></svg>
          </div>
          ...
          >
            <MiniBtn label="Add backboard note" onClick={() => window.dispatchEvent(new CustomEvent("echo-add-note"))}>
      ...
      <div style={{ position: "absolute", inset: 0, pointerEvents: "none" }}>
        {notes.map((n) => (
          <BackboardNote
            ...
            onSendToWhiteboard={() => {
              window.dispatchEvent(new CustomEvent("whiteboard-add-sticky", { detail: { title: n.title, text: n.text, color: "#ffd45b" } }));
  ...
  return (
    <button
      ...
      style={{
        ...
        background: "rgba(10,16,28,.72)", color: "#d7f6ff", cursor: "pointer",
        boxShadow: "0 6px 16px rgba(0,0,0,.4), inset 0 0 0 1px rgba(255,255,255,.05)",
      }}
    >
      {children}
    </button>
...
function BackboardNote({ note, onChange, onRemove, onSendToWhiteboard }) {
  ...
  const setReminder = () => {
    ...
    else { const t = Date.parse(input); if (!Number.isNaN(t)) due = t; }
    if (!due) return alert("Could not parse time.");
  ...
  return (
    <div
      ...
      className="bb-note"
      style={{
        position: "absolute", left: note.x, top: note.y, width: note.w, height: note.h, pointerEvents: "auto",
        borderRadius: 12, border: "1px solid rgba(22,224,255,.35)",
        ...
        color: "#0a1528", display: "flex", flexDirection: "column", overflow: "hidden",
      }}
    >
      <div className="bbn-h" style={{ fontWeight: 700, padding: "6px 8px", display: "flex", alignItems: "center", gap: 6, cursor: "move",
        borderBottom: "1px solid rgba(22,224,255,.25)", background: "rgba(10,16,28,.55)", color: "#d7f6ff" }}>
        {editingTitle ? (
          <input
      ...
      <div style={{ flex: 1, position: "relative" }}>
        {editingBody ? (
          ...
          <div onDoubleClick={() => setEditingBody(true)} style={{ position: "absolute", inset: 0, padding: 10, fontSize: 14, lineHeight: 1.35, whiteSpace: "pre-wrap", color: "#d7f6ff" }}>
            {note.text || <span style={{ opacity: 0.6 }}>Double-click to edit…</span>}
          </div>
        )}
      </div>

      <div className="bbn-r" style={{ position: "absolute", right: 6, bottom: 6, width: 14, height: 14, borderRadius: 4,
        background: "rgba(22,224,255,.25)", border: "1px solid rgba(22,224,255,.38)", cursor: "nwse-resize" }} />
    </div>
  );
}
...
function IconBtn({ title, onClick, children }) {
  return (
    <button
      ...
      style={{
        height: 22, minWidth: 22, padding: "0 6px", borderRadius: 6,
        border: "1px solid rgba(22,224,255,.28)", background: "rgba(22,224,255,.08)",
        color: "#d7f6ff", cursor: "pointer",
      }}
    >
      {children}
    </button>
