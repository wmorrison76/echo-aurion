...
export function ZeldaButton<T>({ getState, onLoad }:{ getState:()=>T; onLoad:(s:T)=>void; }){
  ...
  return (
    <div className="fixed bottom-4 left-4 z-[1200]">
      <button className="px-3 py-2 rounded-lg bg-white/6 border border-white/12" onClick={()=>setOpen(v=>!v)}>
        Snapshots
      </button>
      {open && (
        <div className="mt-2 w-[300px] rounded-xl border border-white/12 bg-[rgba(13,18,29,.98)] p-2">
          ...
          <div className="max-h-64 overflow-auto space-y-1">
            {list.map(s => (
              <button key={s.id}
                className="w-full text-left px-2 py-1 rounded hover:bg-white/6"
                onClick={()=>{ onLoad(s.payload as any); setOpen(false); }}>
